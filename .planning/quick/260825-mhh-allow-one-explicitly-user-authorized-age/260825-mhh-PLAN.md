...
must_haves:
  truths:
    - "The user's 2026-08-25 authorization permits exactly one attempted agent invocation of the fixed seven-source collector under authorization reference `quick-260825-mhh`; an atomic durable consumption record is created before transport and survives failure, cleanup, and deletion of collected outputs, so it cannot become standing permission."
    ...
    - "The agent invokes the production collector once, stops without another attempt on any failure, and independently verifies the completed primary, backup, metadata, rulebook evidence, and source-set root hash."
  ...
  artifacts:
    - path: "scripts/collect-private-authority.ps1"
      provides: "Atomic pre-transport consumption of the closed one-run authorization reference and truthful agent-run acquisition evidence without changing the collector manifest or transport"
    ...
    - path: ".local/authority/authorizations/quick-260825-mhh.consumed.json"
      provides: "Ignored durable evidence that the sole agent authorization was consumed before any publisher request, retained independently of collected outputs"
    - path: "tests/authority/private-authority-collector.test.ts"
      provides: "Synthetic proof that only the exact authorization reference selects the agent-run receipt and all prior collector boundaries remain enforced"
  ...
  key_links:
    ...
    - from: "scripts/collect-private-authority.ps1"
      ...
      via: "FileMode.CreateNew before transport on every authorized-agent entry path; cleanup never removes the record"
      pattern: "quick-260825-mhh.consumed.json"
    - from: "scripts/collect-private-authority.ps1"
...
<objective>
Permit and execute the single fixed private authority collection explicitly authorized by the user on 2026-08-25, then independently verify its source set without weakening any other acquisition, use, or distribution boundary.

Purpose: Finish the missing Phase 1 authority input safely and truthfully: the executor is the actor for this one approved run, while recurring or otherwise agent-run collection remains blocked.
...
<task type="auto" tdd="true">
  <name>Task 1: Consume the one-run authorization and add a private-boundary gate</name>
  <read_first>
    - AGENTS.md
    - .planning/phases/01-rules-and-data-authority/01-CONTEXT.md
    - docs/external-reuse-policy.md
  ...
  <behavior>
    ...
    - Supplying exact authorization reference `quick-260825-mhh` atomically creates `.local/authority/authorizations/quick-260825-mhh.consumed.json` with `FileMode.CreateNew` before transport, selects `user-authorized-agent-run-one-shot-powershell`, and records that reference in the ignored lock after successful verification.
    - The consumed record remains after success, controlled failure, collector cleanup, and deletion of primary/backup/lock outputs; every later use of `quick-260825-mhh`, including a concurrent attempt, fails before a request or output mutation.
    - Any other nonempty authorization reference fails before network or filesystem mutation; the direct wrapper has no generic authorization/config file, recurring mode, URL, manifest, artwork, retry, or scheduling input.
  ...
  <action>Extend `tests/authority/private-authority-collector.test.ts` first. Keep the existing user-run cases unchanged. For the exact `quick-260825-mhh` path, prove a controlled loopback stop immediately after authorization consumption observes the durable record before the server receives any request; prove its strict schema contains only `schemaVersion: 1`, the fixed revision, `user-authorized-agent-run-one-shot-powershell`, `quick-260825-mhh`, and a valid UTC consumed timestamp, with no private root or remote locator. Cover a successful run and a failure after transport begins, then perform the collector's normal cleanup and explicitly delete any primary, backup, and lock outputs; in both cases a second invocation with the same reference must fail before the request count or filesystem changes while the consumed record remains. Include a concurrent two-invocation case proving atomic create-new semantics allow at most one attempt to cross the guard. Prove an unknown, blank-padded, or noncanonical reference is rejected before any request, consumption record, or output mutation. Assert from the PowerShell AST/signatures that the direct script wrapper, exported production function, loopback seam, and collection core all forward the same closed authorization context and that no authorized-agent entry point bypasses the consumption guard. Retain the direct parameter-surface assertion forbidding production URL, descriptor, manifest, art, scheduler, polling, concurrency, retry, timeout, update, or consumption-record override inputs.

Update `scripts/collect-private-authority.ps1` so optional `UserAuthorizationReference` is accepted only as either absent or exactly `quick-260825-mhh`; keep `BackupRoot` and `AcknowledgePrivateUseRisk` mandatory. Route the direct wrapper, `Invoke-PrivateAuthorityCollection`, `Invoke-PrivateAuthorityCollectionForLoopbackTest`, and the core through one acquisition-context discriminator. With no reference, preserve the existing user-run method and do not create an authorization record. With the exact reference, atomically create `.local/authority/authorizations/quick-260825-mhh.consumed.json` using `FileMode.CreateNew` after validating configuration/paths but before calling any transport; fail closed if the file or a same-named filesystem object already exists. Never remove, reset, overwrite, quarantine, or infer eligibility from primary/backup/lock existence. Keep the consumption record outside all staging/output cleanup paths. Pass the closed context into receipt creation so a successful ignored lock records `acquisitionMethod: user-authorized-agent-run-one-shot-powershell` and `authorizationReference: quick-260825-mhh`; a failed attempt has no lock but remains consumed. Add only a loopback-controlled fault immediately after consumption to prove ordering; production still forbids fault injection. Do not change the revision ID, seven descriptors, URLs/hosts, transport, bounds, stop conditions, acknowledgment, backup/verifier gates, source-set identity document, or receipt-last publication.
...
|---|---|
| user authorization -> agent CLI | The committed quick plan authorizes only reference `quick-260825-mhh`; the reference is audit evidence, not cryptographic proof against a repository owner changing code. |
...
2. Structurally assert the three policy/plan contracts encode exactly the two allowed method/reference pairs, 01-07 is autonomous with no `user_setup` or human checkpoint, every stale actor-only statement is removed/qualified, and all other agent acquisition remains blocked.
3. Prove primary, lock, consumption record, and privately selected backup destinations are initially absent and `.local/authority` is ignored/untracked/unstaged, then invoke production exactly once with reference `quick-260825-mhh`; never retry.
...
<success_criteria>
- Only authorization reference `quick-260825-mhh` can label an agent-run receipt; atomic durable consumption before transport makes it one attempted invocation even after failure, cleanup, or output deletion, with no renewed or recurring permission.
