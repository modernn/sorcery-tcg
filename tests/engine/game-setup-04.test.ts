...
import { opaqueActionId, type StateHash } from '../../src/engine/contract.ts';
import {
  createGameManifest,
...
test('RULE-03 Aura occupies any canonical 2x2 area and grounds site minions for three controller turns', async () => {
  ...
  await withSetup(gameManifest, async (ctx) => {
    ...
    assert.equal((await ctx.observe('north')).realm.auras?.[0]?.turnCounters, 1);
    ...
    await take(({ descriptor }) => descriptor.kind === 'draw' && descriptor.zone === 'spellbook');
    await take(({ descriptor }) => descriptor.kind === 'end-turn');
    assert.equal((await ctx.observe('north')).realm.auras?.[0]?.turnCounters, 2);
    await take(({ descriptor }) => descriptor.kind === 'draw' && descriptor.zone === 'spellbook');
    await take(({ descriptor }) => descriptor.kind === 'end-turn');
    await take(({ descriptor }) => descriptor.kind === 'draw' && descriptor.zone === 'spellbook');
    await take(({ descriptor }) => descriptor.kind === 'end-turn');
...
// Bearer-cell Artifact release on oversized death is covered by Rust
// `carried_artifacts_should_ride_one_named_cell_of_an_oversized_bearer` and this SetupCtx play path.
test('RULE-03 oversized minions occupy one canonical 2x2 footprint for movement, combat, Auras, and terrain', async () => {
  ...
  await withSetup(gameManifest, async (ctx) => {
    ...
    const endAndDraw = async (zone: 'atlas' | 'spellbook' = 'spellbook'): Promise<void> => {
      await take(({ descriptor }) => descriptor.kind === 'end-turn');
      await take(({ descriptor }) => descriptor.kind === 'draw' && descriptor.zone === zone);
    };
    ...
    const magicRoot = await ctx.checkpoint();
    const teleportActions = (await ctx.legalActions('north')).filter(({ descriptor }) =>
    ...
    const teleported = await ctx.step(shiftedTeleport);
    ...
    assert.equal(await ctx.verifyReplay(), true);
    const leapActions = (await ctx.legalActions('north'))
    ...
    const leaped = await ctx.step(focusedLeap);
    ...
    await ctx.resume(magicRoot.checkpoint);
    const blinkActions = (await ctx.legalActions('north')).filter(({ descriptor }) =>
      descriptor.kind === 'cast-magic'
    ...
    const blinked = await ctx.step(shiftedBlink);
    ...
    assert.deepEqual(translated.descriptor.kind === 'move-and-attack'
      ...
      : [], ['B3', 'B2']);
    await take(({ actionId }) => actionId === translated.actionId);
    ...
    const attackRoot = await ctx.checkpoint();
    const declined = await ctx.step(await ctx.action(({ descriptor }) =>
    ...
    const intercepted = await ctx.step(await ctx.action(({ descriptor }) =>
      descriptor.kind === 'intercept'
    ...
    await ctx.resume(attackRoot.checkpoint);
    ...
    await take(({ descriptor }) => descriptor.kind === 'close-defend'
...
test('RULE-03 Humble Village Genesis may spend its mana to summon one Foot Soldier', async () => {
  ...
  await withSetup(gameManifest, async (ctx) => {
    ...
    const declinedResult = await ctx.step(declined);
...
test('RULE-02/03 Geomancer creates Rubble and privately replaces it with the top Atlas site', async () => {
  ...
  await withSetup(gameManifest, async (ctx) => {
    ...
    assert.ok(top);
    const before = canonicalJson((await ctx.observe('north')) as unknown as JsonValue);
    ...
    const revealed = canonicalJson((await ctx.observe('south')) as unknown as JsonValue);
...
test('RULE-02/03 site Genesis resumes after ordered terrain-replacement Deathrites', async () => {
  ...
  const playToDestruction = async (
    ctx: SetupCtx,
    deathriteCount: 1 | 2,
  ): Promise<Readonly<{ deathrites: readonly GameSession['state']['realm']['units'][number][];
    top: GameSession['state']['players']['north']['atlas'][number] }>> => {
    await ctx.keep();
    await ctx.keep();
    await takeAction(ctx, ({ descriptor }) => descriptor.kind === 'play-site'
  ...
  await withSetup(gameManifest, async (ctx) => {
    ...
    assert.equal(interrupted.session.state.stateVersion, beforeInterruptVersion + 1);
    ...
    assert.equal((await ctx.legalActions('south')).length, 0);
    assert.deepEqual(
      (await ctx.legalActions('north')).map(({ actionId }) => actionId),
      orders.map(({ actionId }) => actionId),
    );
    const branchCheckpoint = createGameCheckpoint(ctx.session);
    const branchVersion = ctx.state.stateVersion;
    const branchHashes: StateHash[] = [];
    for (const order of orders) {
      ...
      const resolved = await ctx.step(order);
      assert.equal(resolved.accepted, true);
      if (!resolved.accepted) throw new Error('expected site Genesis to resume');
      assert.equal(resolved.session.state.stateVersion, branchVersion + 1);
  ...
  await withSetup(gameManifest, async (ctx) => {
    const { deathrites: immediateDeathrites } = await playToDestruction(ctx, 1);
    const immediateVersion = ctx.state.stateVersion;
    ...
    assert.equal(immediateResult.session.state.stateVersion, immediateVersion + 1);
    ...
    assert.equal(immediateResult.session.state.players.north.cemetery.some(({ instanceId }) =>
      instanceId === immediateDeathrites[0]!.instanceId), true);
    assert.equal(await ctx.verifyReplay(), true);
  });
...
test('RULE-03 seasonal River Genesis privately keeps or bottoms the next spell', async () => {
  ...
  await withSetup(gameManifest, async (ctx) => {
    ...
    const southBefore = canonicalJson((await ctx.observe('south')) as unknown as JsonValue);
    ...
    const beforePlayVersion = ctx.state.stateVersion;
    ...
    const pendingSouth = canonicalJson((await ctx.observe('south')) as unknown as JsonValue);
    assert.equal(pendingSouth.includes(top.cardId), false);
    assert.equal(pendingSouth.includes(top.instanceId), false);

    const genesisCheckpoint = createGameCheckpoint(ctx.session);
    const choices = await ctx.legalActions('north');
    ...
    const kept = await ctx.step(keepNext);
    assert.equal(kept.accepted, true);
    if (!kept.accepted) return;
    assert.equal(kept.session.state.stateVersion, beforePlayVersion + 2);
    ...
    const keptSouthView = canonicalJson((await ctx.observe('south')) as unknown as JsonValue);
    assert.equal(await ctx.verifyReplay(), true);

    await ctx.resume(genesisCheckpoint);
    const bottomed = await ctx.step(bottomNext);
    assert.equal(bottomed.accepted, true);
    if (!bottomed.accepted) return;
    assert.equal(bottomed.session.state.stateVersion, beforePlayVersion + 2);
    ...
    assert.equal(
      keptSouthView,
      canonicalJson((await ctx.observe('south')) as unknown as JsonValue),
    );
    assert.equal(await ctx.verifyReplay(), true);
  });
});
...
test('RULE-03 Observatory privately reorders the next three spells without drawing', async () => {
  ...
  await withSetup(gameManifest, async (ctx) => {
    ...
    assert.ok(identity);
    assert.ok(reverse);
    const kept = await ctx.step(identity);
    assert.equal(kept.accepted, true);
    if (!kept.accepted) return;
    assert.deepEqual(kept.session.state.players.north.spellbook, before);
    const keptSouthView = canonicalJson((await ctx.observe('south')) as unknown as JsonValue);
    ...
    await ctx.resume(genesisCheckpoint);
    const reversed = await ctx.step(reverse);
...
test('RULE-03/04 an undamaged 0/0 Genesis minion survives until it takes positive damage', async () => {
  await withNorthAttacksAtC2({
    ...
    const target = (await ctx.observe('north')).realm.units
...
test('RULE-04 an active minion prevents damage from a unit at its current-power threshold', async () => {
  for (const [seed, sourcePower, disabled, expectedDamage] of [
    ...
    await withNorthAttacksAtC2({
      ...
      assert.equal(fought.accepted, true);
      if (!fought.accepted) return;
      const target = (await ctx.observe('south')).realm.units.find(({ instanceId }) =>
        instanceId === targetInstanceId);
...
test('RULE-04 attacking-only first strike resolves deaths before normal strikes and is inactive while defending', async () => {
  ...
  await withNorthAttacksAtC2({
    ...
    spell: firstStrike,
    southSpell: vanilla,
  }, async (ctx, { attackerInstanceId, targetInstanceId }) => {
    await takeAction(ctx, ({ descriptor }) =>
      descriptor.kind === 'declare-attack'
        && descriptor.target.kind === 'minion'
        && descriptor.target.instanceId === targetInstanceId);
  ...
  await withNorthAttacksAtC2({
    seed: 280,
    spell: { ...firstStrike, attack: 6, defense: 10 },
    avatar: { attack: 2, defense: 1, drawSpell: false, life: 20 },
    southSpell: deathrite,
    extraSouthMinionsAtC1: 1,
  }, async (ctx, { attackerInstanceId, targetInstanceId }) => {
