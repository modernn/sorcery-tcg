  ...
  await withSetup(gameManifest, async (ctx) => {
    ...
    const castCaveIn = async (bearer: 'avatar' | 'minion'): Promise<void> => {
      ...
      assert.deepEqual(casts.flatMap(({ descriptor }) => descriptor.kind === 'cast-magic'
        ...
        : []), ['C1:surface']);
      // Both probes here forge state legal play cannot reach: a Rubble injected at a specific
      // cell, and the north Avatar's region forced to 'underground' (the Rust `AvatarPosition`
      // carries no region field at all, so that state is not representable there). Both facts
      // are proven directly in Rust instead: Cave-In legally targeting Rubble alongside a land
      // site is `rule_catalog_0044_cave_in_minion_slice_should_burrow_in_canonical_order`
      // (crates/sorcery-engine/tests/magic_rules.rs), and the caster-region gate that empties
      // Cave-In's target choices for an underground caster is
      // `burrow_all_minions_magic_should_require_a_surface_caster`
...
test('RULE-03/04 Waterbound derives Disabled from terrain and survives only with active abilities', async () => {
  ...
  await withSetup(gameManifest, async (ctx) => {
    ...
    const northSecondMain = async (): Promise<void> => {
      await takeAction(ctx, ({ descriptor }) => descriptor.kind === 'end-turn');
      await takeAction(ctx, ({ descriptor }) =>
        descriptor.kind === 'draw' && descriptor.zone === 'spellbook');
      await takeAction(ctx, ({ descriptor }) =>
    ...
    // --- underwater branch: summon underwater, then destroy the site under it ---
    await takeAction(ctx, ({ descriptor }) =>
      descriptor.kind === 'summon-minion'
        && descriptor.cardInstanceId === waterbound.instanceId
    ...
    assert.equal(ctx.state.players.north.atlas.length, atlasBefore);
    assert.equal(ctx.state.players.north.hand.atlas.length, atlasHandBefore);
    assert.equal(destroyed.receipt.events.some(({ type }) => type === 'site-drawn'), false);
    assert.equal(await ctx.verifyReplay(), true);

    // --- surface branch: summon on the surface and confirm Disabled survival rules ---
    await ctx.resume(cp);
    await takeAction(ctx, ({ descriptor }) =>
      descriptor.kind === 'summon-minion'
        && descriptor.cardInstanceId === waterbound.instanceId
...
test('RULE-03 site Genesis heals every nearby Avatar through shared life caps', async () => {
  ...
  let northAtlas: readonly string[] = [];
  let southAtlas: readonly string[] = [];
  await withPreview(base, async (preview) => {
    northAtlas = preview.state.players.north.hand.atlas.map(({ cardId }) => cardId);
    southAtlas = preview.state.players.south.hand.atlas.map(({ cardId }) => cardId);
  });
  const [plainC4, plainC3, holyGround] = northAtlas;
  const [plainC1, plainC2] = southAtlas;
  assert.ok(plainC4);
  assert.ok(plainC3);
  ...
  await withSetup(gameManifest, async (ctx) => {
    ...
    const southTurn = createGameCheckpoint(ctx.session);
    const southAvatarInstanceId = ctx.state.players.south.avatar.card.instanceId;

    await take(({ descriptor }) => descriptor.kind === 'end-turn');
    await take(({ descriptor }) => descriptor.kind === 'draw' && descriptor.zone === 'spellbook');
    const farResult = await ctx.step(await ctx.action(({ descriptor }) =>
      descriptor.kind === 'play-site'
    ...
    await take(({ descriptor }) => descriptor.kind === 'end-turn');
    await take(({ descriptor }) => descriptor.kind === 'draw' && descriptor.zone === 'spellbook');
    const nearResult = await ctx.step(await ctx.action(({ descriptor }) =>
      descriptor.kind === 'play-site'
        && descriptor.cardId === holyGround
        && descriptor.cell === 'B3'));
...
test('RULE-03 site Genesis makes units at nearby sites Immobile until its controller next turn', async () => {
  ...
  let northAtlas: readonly string[] = [];
  let southAtlas: readonly string[] = [];
  let northSpells: readonly string[] = [];
  let southSpells: readonly string[] = [];
  await withPreview(base, async (preview) => {
    northAtlas = preview.state.players.north.hand.atlas.map(({ cardId }) => cardId);
    southAtlas = preview.state.players.south.hand.atlas.map(({ cardId }) => cardId);
    northSpells = preview.state.players.north.hand.spellbook.map(({ cardId }) => cardId);
    southSpells = preview.state.players.south.hand.spellbook.map(({ cardId }) => cardId);
  });
  const [plainC4, plainC3, quagmire] = northAtlas;
  const sinkhole = southAtlas[0];
  const [casterA, casterB] = northSpells;
  const disabledEnemy = southSpells[0];
  assert.ok(plainC4);
  assert.ok(plainC3);
  assert.ok(quagmire);
  assert.ok(sinkhole);
