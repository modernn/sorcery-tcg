...
<body>
  ...
  <script>
    ...
    function actionGroup(kind){if(kind==='draw'||kind==='draw-site'||kind==='draw-spell')return'Draw';if(kind==='play-site')return'Play a site';if(kind==='summon-minion'||kind==='cast-magic'||kind==='cast-artifact')return'Play a card';if(kind.startsWith('activate-')||kind==='pick-up-artifacts'||kind==='drop-artifacts')return'Use an ability';if(kind==='move-and-attack')return'Move / attack';if(kind==='declare-attack'||kind==='decline-attack'||kind==='defend'||kind==='close-defend'||kind==='intercept'||kind==='close-intercept'||kind==='allocate-strike')return'Combat';if(kind==='end-turn')return'End turn';return'Resolve effect'}
