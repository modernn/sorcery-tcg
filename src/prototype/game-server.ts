...
<body>
  ...
  <script>
    ...
    function cardChip(card){return '<span class="card" title="'+card.cardId+'">'+card.cardId+'</span>'}
    function renderPlayer(view,owner){var player=view.players[owner],hand=player.hand;byId(owner+'-stats').textContent='Atlas '+player.atlasCount+' · Spellbook '+player.spellbookCount+' · Mana '+player.mana+(player.domainEstablished?' · Domain established':' · Domain pending');var cards=[];['atlas','spellbook'].forEach(function(zone){var value=hand[zone];if(Array.isArray(value)){value.forEach(function(card){cards.push(cardChip(card))})}else{cards.push('<span class="card">'+value+' hidden '+zone+'</span>')}});byId(owner+'-hand').innerHTML=cards.join('')}
    function renderRealm(view){document.querySelectorAll('[data-cell]').forEach(function(cell){cell.innerHTML=''});Object.entries(view.realm.sites||{}).forEach(function(entry){var cell=document.querySelector('[data-cell="'+entry[0]+'"]');if(cell)cell.innerHTML+='<span class="piece site">'+entry[1].cardId+'</span>'});['north','south'].forEach(function(owner){var avatar=view.players[owner].avatar,cell=document.querySelector('[data-cell="'+avatar.location+'"]');if(cell)cell.innerHTML+='<span class="piece avatar '+owner+'">'+owner+' avatar'+(avatar.tapped?' · tapped':'')+'</span>'})}
    function actionButton(candidate){var button=document.createElement('button');button.type='button';button.className='action';button.textContent=candidate.label;button.title=JSON.stringify(candidate.descriptor);button.addEventListener('click',function(){submit(candidate.actionId)});return button}
    function renderActions(actions,phase){var dock=byId('actions');dock.innerHTML='';if(!actions.length){dock.innerHTML='<p class="empty">No legal actions for this observer.</p>';return}if(phase==='mulligan'){var keep=actions.filter(function(a){return a.descriptor.kind==='mulligan'&&!a.descriptor.atlasOrder.length&&!a.descriptor.spellbookOrder.length});keep.forEach(function(a){dock.appendChild(actionButton(a))});var rest=actions.filter(function(a){return keep.indexOf(a)<0});var details=document.createElement('details'),summary=document.createElement('summary'),list=document.createElement('div');summary.textContent='Show '+rest.length+' mulligan alternatives';list.className='actions';rest.forEach(function(a){list.appendChild(actionButton(a))});details.append(summary,list);dock.appendChild(details);return}actions.forEach(function(a){dock.appendChild(actionButton(a))})}
    function render(data){snapshot=data;var view=data.view;byId('observer').textContent=seat;byId('active').textContent=view.activeSeat;byId('phase').textContent='Turn '+view.turnNumber+' · '+view.phase;byId('version').textContent=view.stateVersion;byId('hash').textContent=data.stateHash;renderPlayer(view,'north');renderPlayer(view,'south');renderRealm(view);renderActions(data.actions,view.phase)}
...
export function createGamePrototypeServer(initialSeed = 1): Server {
  let session: GameSession = createGameSession(createSyntheticDemoManifest(initialSeed));
  ...
  return createServer(async (request, response) => {
    try {
      ...
      if (request.method === 'POST' && url.pathname === '/api/reset') {
        ...
        session = createGameSession(createSyntheticDemoManifest(body.seed as number));
...
if (process.argv[1] && resolve(process.argv[1]) === fileURLToPath(import.meta.url)) {
  createGamePrototypeServer().listen(DEFAULT_PORT, HOST, () => {
