...
pub enum MagicEffect {
    ...
    DestroyOwnArtifactAtLocationForAreaDamage(u8),
...
pub struct MinionFacts {
    ...
    pub nearby_avatars_may_discard_card_to_gain_control_of_this: bool,
...
fn parse_magic(object: &Map<String, Value>, path: &str) -> Result<MagicFacts, FactError> {
    ...
    let effect = one_effect(
        [
            ...
            fixed_integer(object, "destroyOwnArtifactAtLocationForAreaDamage", 3, path)?
                .then_some(MagicEffect::DestroyOwnArtifactAtLocationForAreaDamage(3)),
            true_only(
                object,
                "destroyUndeadMinionsAndArtifactsAtLocationWithinTwoSteps",
                path,
            ...
            true_only(object, "drawSiteThenMayPlayLandSite", path)?
                .then_some(MagicEffect::DrawSiteThenMayPlayLandSite),
            true_only(object, "drawSiteThenMayPlayWaterSite", path)?
                .then_some(MagicEffect::DrawSiteThenMayPlayWaterSite),
            ...
            true_only(object, "gainControlOfTargetEnemyMinionThisTurn", path)?
                .then_some(MagicEffect::GainControlOfTargetEnemyMinionThisTurn),
            true_only(
                object,
                "gainControlOfTargetEnemyMinionUntilStealthLost",
                path,
            )?
            .then_some(MagicEffect::GainControlOfTargetEnemyMinionUntilStealthLost),
            ...
            true_only(object, "grantAirborneToAllyThisTurnThenDrawSpell", path)?
                .then_some(MagicEffect::GrantAirborneToAllyThisTurnThenDrawSpell),
            true_only(object, "grantAirborneToTargetMinion", path)?
                .then_some(MagicEffect::GrantAirborneToTargetMinion),
            true_only(object, "grantChargeToAllyThisTurn", path)?
                .then_some(MagicEffect::GrantChargeToAllyThisTurn),
            true_only(object, "grantDoubleDamageToAllyNextStrikeThisTurn", path)?
                .then_some(MagicEffect::GrantDoubleDamageToAllyNextStrikeThisTurn),
            ...
            true_only(object, "grantLethalToAllyThisTurnThenDrawSpell", path)?
                .then_some(MagicEffect::GrantLethalToAllyThisTurnThenDrawSpell),
            true_only(object, "grantMovementOneToAllyThisTurnThenDrawSpell", path)?
                .then_some(MagicEffect::GrantMovementOneToAllyThisTurnThenDrawSpell),
            ...
            true_only(object, "grantStealthToAlliedMinionsThenDrawSpell", path)?
                .then_some(MagicEffect::GrantStealthToAlliedMinionsThenDrawSpell),
            true_only(
                object,
                "grantStealthToAlliedMinionOccupyingEnemySiteThenDrawSpell",
                path,
            )?
            .then_some(MagicEffect::GrantStealthToAlliedMinionOccupyingEnemySiteThenDrawSpell),
            ...
            true_only(object, "wardEachAlliedMinionAtTargetWaterSite", path)?
                .then_some(MagicEffect::WardEachAlliedMinionAtTargetWaterSite),
            true_only(object, "wardNearbyMinionOrSite", path)?
                .then_some(MagicEffect::WardNearbyMinionOrSite),
            ...
            .then_some(MagicEffect::ReturnUpToThreeCemeteryCardsToDeckBottomThenDrawSpell),
            ...
            true_only(object, "allyStrikesEachEnemyAtItsLocation", path)?
                .then_some(MagicEffect::AllyStrikesEachEnemyAtItsLocation),
            true_only(object, "allyTakesUpToTwoSteps", path)?
                .then_some(MagicEffect::AllyTakesUpToTwoSteps),
            ...
            true_only(object, "silenceAndTapNearbyMinionThenMayDrawSpell", path)?
                .then_some(MagicEffect::SilenceAndTapNearbyMinionThenMayDrawSpell),
...
fn parse_minion(object: &Map<String, Value>, path: &str) -> Result<MinionFacts, FactError> {
    ...
    Ok(MinionFacts {
        ...
        nearby_avatars_may_discard_card_to_gain_control_of_this: true_only(
            object,
            "nearbyAvatarsMayDiscardCardToGainControlOfThis",
            path,
        )?,
