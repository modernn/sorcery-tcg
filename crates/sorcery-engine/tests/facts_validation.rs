...
fn parse_should_accept_every_artifact_aura_and_magic_effect_shape() {
    ...
    let magic_effects = [
        ...
        ("disableTargetMinionWithinTwoStepsUntilDamaged", json!(true)),
        ...
        ("grantAirborneToAllyThisTurnThenDrawSpell", json!(true)),
        ("grantAirborneToTargetMinion", json!(true)),
        ("grantChargeToAllyThisTurn", json!(true)),
        ("grantDoubleDamageToAllyNextStrikeThisTurn", json!(true)),
        ...
        ("grantMovementOneToAllyThisTurnThenDrawSpell", json!(true)),
...
fn typed_effects_should_retain_only_normalized_values() {
    ...
    .expect("valid stacked start-turn draw and mill minion") else {
        panic!("expected minion facts");
    };
...
#[test]
fn genesis_disable_with_lose_controller_life_should_parse() {
    let CardFacts::Minion(facts) = parse_card_definition(
        "disable-lose-life",
        &with(
    ...
    .expect("valid disable plus lose-life minion") else {
        panic!("expected minion facts");
    };
...
fn genesis_disable_with_targeted_damage_should_parse() {
    ...
    .expect("valid disable plus targeted damage minion") else {
        panic!("expected minion facts");
    };
    assert!(facts.genesis_disable_self_until_damaged);
...
#[test]
fn genesis_disable_with_draw_spells_should_parse() {
    let CardFacts::Minion(facts) = parse_card_definition(
        "disable-draw-spells",
        &with(
            with(minion(), "genesisDisableSelfUntilDamaged", json!(true)),
    ...
    .expect("valid disable plus draw-spells minion") else {
        panic!("expected minion facts");
    };
    assert!(facts.genesis_disable_self_until_damaged);
    assert_eq!(facts.genesis_draw_spells, Some(1));
...
fn genesis_disable_with_draw_site_should_parse() {
    ...
    .expect("valid disable plus draw-site minion") else {
        panic!("expected minion facts");
    };
    assert!(facts.genesis_disable_self_until_damaged);
...
#[test]
fn ward_each_allied_minion_at_target_water_site_should_parse() {
    let CardFacts::Magic(facts) = parse_card_definition(
        "ward-each-ally-at-water-site",
        &spell(
            "magic",
            ("wardEachAlliedMinionAtTargetWaterSite", json!(true)),
        ),
    )
    .expect("valid ward-each-allied-minion-at-water-site Magic") else {
        panic!("expected Magic facts");
    };
    assert_eq!(
        facts.effect,
...
#[test]
fn grant_stealth_to_allied_minion_occupying_enemy_site_then_draw_spell_should_parse() {
    let CardFacts::Magic(facts) = parse_card_definition(
        "grant-stealth-enemy-site-then-draw",
        &spell(
            "magic",
            (
                "grantStealthToAlliedMinionOccupyingEnemySiteThenDrawSpell",
    ...
    .expect("valid grant-Stealth-on-enemy-site then draw-spell Magic") else {
        panic!("expected Magic facts");
    };
    assert_eq!(
        facts.effect,
