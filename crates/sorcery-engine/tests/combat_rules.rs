...
fn attack_deathrite_manifest(seed: u32) -> String {
    let mut value = json!({
        "authority": {
            "contentHash": identity_hash(&json!({ "fixture": "combat-declare-attack-deathrite-withheld" }))
                .expect("synthetic authority identity"),
            "mode": "synthetic",
            "revisionId": "synthetic-combat-declare-attack-deathrite-withheld-v1",
        },
        "cards": {
            "north-attacker": minion_card(json!({
                "defense": 2,
            ...
            "north-avatar": avatar_card(),
            "north-deathrite": minion_card(json!({
                "deathriteDrawSite": true,
                "defense": 1,
            })),
            "north-rain": rain_card(),
            "north-site": site_card(json!({})),
            "south-avatar": avatar_card(),
            "south-minion": minion_card(json!({})),
            "south-site": site_card(json!({})),
        },
        "decks": {
            "north": {
    ...
    value["manifestId"] = json!(identity_hash(&value).expect("manifest identity"));
    canonical_json(&value).expect("canonical synthetic declare-attack Deathrite manifest")
}

fn try_end_and_draw(session: &mut Session) -> Option<()> {
...
struct PendingDeclareAttackDeathriteSetup {
    attacker_id: String,
    deathrite_ids: [String; 2],
    session: Session,
}

fn try_pending_declare_attack_during_deathrite_order(
    encoded: &str,
) -> Option<PendingDeclareAttackDeathriteSetup> {
    try_pending_declare_attack_during_deathrite_order_inner(encoded).ok()
}

fn try_pending_declare_attack_during_deathrite_order_inner(
    encoded: &str,
) -> Result<PendingDeclareAttackDeathriteSetup, &'static str> {
    let mut session = Session::new(encoded).map_err(|_| "session")?;
    keep(&mut session);
    keep(&mut session);
    ...
    .ok_or("first-deathrite")?;
    let second = try_accept_where(&mut session, |descriptor| {
        descriptor["kind"] == "summon-minion"
            && descriptor["cardId"] == "north-deathrite"
            && descriptor["cell"] == "C4"
    })
    .ok_or("second-deathrite")?;
    ...
    .ok_or("attacker")?;
    let attacker_id = attacker.0["cardInstanceId"]
        .as_str()
        .ok_or("attacker-id")?
        .to_owned();
    try_end_and_draw(&mut session).ok_or("end-draw-1")?;
    try_accept_where(&mut session, |descriptor| {
        descriptor["kind"] == "play-site" && descriptor["cell"] == "C1"
    })
    .ok_or("play-c1")?;
    try_end_and_draw_spellbook(&mut session).ok_or("end-draw-2")?;
    try_accept_where(&mut session, |descriptor| {
        descriptor["kind"] == "play-site" && descriptor["cell"] == "C3"
    })
    .ok_or("play-c3")?;
    try_end_and_draw(&mut session).ok_or("end-draw-3")?;
    try_accept_where(&mut session, |descriptor| {
        descriptor["kind"] == "play-site" && descriptor["cell"] == "C2"
    })
    .ok_or("play-c2")?;
    try_end_and_draw_spellbook(&mut session).ok_or("end-draw-4")?;
    try_accept_where(&mut session, |descriptor| {
    ...
    if combat_unit(
        &wounded,
        first.0["cardInstanceId"].as_str().ok_or("first-id")?,
    )["damage"]
        != 1
        || combat_unit(
            &wounded,
            second.0["cardInstanceId"].as_str().ok_or("second-id")?,
        )["damage"]
            != 1
    {
        return Err("undamaged");
    }
    try_accept_where(&mut session, |descriptor| {
        descriptor["kind"] == "move-and-attack"
            && descriptor["unitInstanceId"] == attacker_id
    ...
    if paused["pendingDeathrites"]["returnPhase"] != "attack" {
        return Err("return-phase");
    }
    if offers_kind(&session, "declare-attack") {
        return Err("declare-attack-offered");
    }
    let mut deathrite_ids = [
        first.0["cardInstanceId"]
            .as_str()
            .ok_or("first-id")?
            .to_owned(),
        second.0["cardInstanceId"]
            .as_str()
            .ok_or("second-id")?
            .to_owned(),
    ];
    deathrite_ids.sort_unstable();
    Ok(PendingDeclareAttackDeathriteSetup {
        attacker_id,
        deathrite_ids,
        session,
    })
...
fn declare_attack_deathrite_seed_with(start: u32) -> String {
    (start..start + 2048)
        .map(attack_deathrite_manifest)
        .find(|candidate| try_pending_declare_attack_during_deathrite_order(candidate).is_some())
        .expect(
            "bounded seed that reaches pending Deathrites while Attack would offer declare-attack",
        )
}

#[test]
fn rule_catalog_1153_declare_attack_withheld_during_pending_deathrite_order() {
    let encoded = declare_attack_deathrite_seed_with(1153);
    ...
    let resumed = state(session);
    assert_eq!(resumed["phase"], "attack");
    assert_eq!(resumed["decisionSeat"], "north");
    assert!(resumed["pendingDeathrites"].is_null());
    assert!(
