...
fn deathrite_loss_gain_stack_withheld_manifest(seed: u32) -> String {
    let mut value = json!({
        "authority": {
            "contentHash": identity_hash(&json!({ "fixture": "start-turn-loss-gain-deathrite-withheld" }))
                .expect("synthetic authority identity"),
            "mode": "synthetic",
            "revisionId": "synthetic-start-turn-loss-gain-deathrite-withheld-v1",
        },
        "cards": {
            "north-avatar": avatar(),
            "north-pulser": minion(json!({
...
#[test]
fn rule_catalog_1213_loss_then_gain_start_turn_trigger_withheld_during_pending_deathrite_order() {
    let encoded = (1213..1213 + 256)
        .map(deathrite_loss_gain_stack_withheld_manifest)
        .find(|candidate| try_pending_deathrite_during_draw_mill_start_turn(candidate).is_some())
        .expect("bounded seed that reaches pending Deathrites during loss-gain start-turn");
    let setup = try_pending_deathrite_during_draw_mill_start_turn(&encoded)
        .expect("complete loss-gain start-turn Deathrite withheld setup");
    assert_stacked_start_turn_withheld(setup);
}

fn deathrite_draw_sites_mill_spells_withheld_manifest(seed: u32) -> String {
    let mut value = json!({
        "authority": {
            "contentHash": identity_hash(&json!({ "fixture": "start-turn-draw-sites-mill-spells-deathrite-withheld" }))
                .expect("synthetic authority identity"),
            "mode": "synthetic",
            "revisionId": "synthetic-start-turn-draw-sites-mill-spells-deathrite-withheld-v1",
        },
        "cards": {
            "north-avatar": avatar(),
            "north-mill-card": minion(json!({})),
...
#[test]
fn rule_catalog_1214_draw_sites_then_mill_spells_start_turn_trigger_withheld_during_pending_deathrite_order()
 {
    let encoded = (1214..1214 + 256)
        .map(deathrite_draw_sites_mill_spells_withheld_manifest)
        .find(|candidate| try_pending_deathrite_during_draw_mill_start_turn(candidate).is_some())
        .expect(
            "bounded seed that reaches pending Deathrites during draw-sites-mill-spells start-turn",
        );
    let setup = try_pending_deathrite_during_draw_mill_start_turn(&encoded)
        .expect("complete draw-sites-mill-spells start-turn Deathrite withheld setup");
    assert_stacked_start_turn_withheld(setup);
}

fn deathrite_draw_spells_mill_sites_withheld_manifest(seed: u32) -> String {
    let mut value = json!({
        "authority": {
            "contentHash": identity_hash(&json!({ "fixture": "start-turn-draw-spells-mill-sites-deathrite-withheld" }))
                .expect("synthetic authority identity"),
            "mode": "synthetic",
            "revisionId": "synthetic-start-turn-draw-spells-mill-sites-deathrite-withheld-v1",
        },
        "cards": {
            "north-avatar": avatar(),
            "north-draw-card": minion(json!({})),
...
#[test]
fn rule_catalog_1215_draw_spells_then_mill_sites_start_turn_trigger_withheld_during_pending_deathrite_order()
 {
    let encoded = (1215..1215 + 256)
        .map(deathrite_draw_spells_mill_sites_withheld_manifest)
        .find(|candidate| {
            try_pending_deathrite_during_draw_sites_mill_start_turn(candidate).is_some()
        })
        .expect(
            "bounded seed that reaches pending Deathrites during draw-spells-mill-sites start-turn",
        );
    let setup = try_pending_deathrite_during_draw_sites_mill_start_turn(&encoded)
        .expect("complete draw-spells-mill-sites start-turn Deathrite withheld setup");
    assert_stacked_start_turn_withheld(setup);
}

fn deathrite_mana_gain_here_damage_withheld_manifest(seed: u32) -> String {
    let mut value = json!({
        "authority": {
            "contentHash": identity_hash(&json!({ "fixture": "start-turn-mana-gain-here-damage-deathrite-withheld" }))
                .expect("synthetic authority identity"),
            "mode": "synthetic",
            "revisionId": "synthetic-start-turn-mana-gain-here-damage-deathrite-withheld-v1",
        },
        "cards": {
            "north-avatar": avatar(),
            "north-pulser": minion(json!({
...
#[test]
fn rule_catalog_1216_mana_gain_then_here_damage_start_turn_trigger_withheld_during_pending_deathrite_order()
 {
    let encoded = (1216..1216 + 256)
        .map(deathrite_mana_gain_here_damage_withheld_manifest)
        .find(|candidate| {
            try_pending_deathrite_during_draw_sites_here_damage_start_turn(candidate).is_some()
        })
        .expect(
            "bounded seed that reaches pending Deathrites during mana-gain-here-damage start-turn",
        );
    let setup = try_pending_deathrite_during_draw_sites_here_damage_start_turn(&encoded)
        .expect("complete mana-gain-here-damage start-turn Deathrite withheld setup");
    assert_stacked_start_turn_withheld(setup);
}

fn deathrite_loss_mana_gain_withheld_manifest(seed: u32) -> String {
    let mut value = json!({
        "authority": {
            "contentHash": identity_hash(&json!({ "fixture": "start-turn-loss-mana-gain-deathrite-withheld" }))
                .expect("synthetic authority identity"),
            "mode": "synthetic",
            "revisionId": "synthetic-start-turn-loss-mana-gain-deathrite-withheld-v1",
        },
        "cards": {
            "north-avatar": avatar(),
            "north-pulser": minion(json!({
...
#[test]
fn rule_catalog_1217_loss_then_mana_gain_start_turn_trigger_withheld_during_pending_deathrite_order()
 {
    let encoded = (1217..1217 + 256)
        .map(deathrite_loss_mana_gain_withheld_manifest)
        .find(|candidate| try_pending_deathrite_during_draw_mill_start_turn(candidate).is_some())
        .expect("bounded seed that reaches pending Deathrites during loss-mana-gain start-turn");
    let setup = try_pending_deathrite_during_draw_mill_start_turn(&encoded)
        .expect("complete loss-mana-gain start-turn Deathrite withheld setup");
    assert_stacked_start_turn_withheld(setup);
}

fn deathrite_mill_spells_lure_withheld_manifest(seed: u32) -> String {
    let mut value = json!({
        "authority": {
            "contentHash": identity_hash(&json!({ "fixture": "start-turn-mill-spells-lure-deathrite-withheld" }))
                .expect("synthetic authority identity"),
            "mode": "synthetic",
            "revisionId": "synthetic-start-turn-mill-spells-lure-deathrite-withheld-v1",
        },
        "cards": {
            "north-avatar": avatar(),
            "north-draw-card": minion(json!({})),
...
#[test]
fn rule_catalog_1223_mill_spells_then_lure_start_turn_trigger_withheld_during_pending_deathrite_order()
 {
    let encoded = (1223..1223 + 256)
        .map(deathrite_mill_spells_lure_withheld_manifest)
        .find(|candidate| try_pending_deathrite_during_draw_lure_start_turn(candidate).is_some())
        .expect("bounded seed that reaches pending Deathrites during mill-spells-lure start-turn");
    let setup = try_pending_deathrite_during_draw_lure_start_turn(&encoded)
        .expect("complete mill-spells-lure start-turn Deathrite withheld setup");
    assert_stacked_start_turn_withheld(setup);
}

fn deathrite_mill_sites_lure_withheld_manifest(seed: u32) -> String {
    let mut value = json!({
        "authority": {
            "contentHash": identity_hash(&json!({ "fixture": "start-turn-mill-sites-lure-deathrite-withheld" }))
                .expect("synthetic authority identity"),
            "mode": "synthetic",
            "revisionId": "synthetic-start-turn-mill-sites-lure-deathrite-withheld-v1",
        },
        "cards": {
            "north-avatar": avatar(),
            "north-pulser": minion(json!({
...
#[test]
fn rule_catalog_1224_mill_sites_then_lure_start_turn_trigger_withheld_during_pending_deathrite_order()
 {
    let encoded = (1224..1224 + 256)
        .map(deathrite_mill_sites_lure_withheld_manifest)
        .find(|candidate| try_pending_deathrite_during_draw_lure_start_turn(candidate).is_some())
        .expect("bounded seed that reaches pending Deathrites during mill-sites-lure start-turn");
    let setup = try_pending_deathrite_during_draw_lure_start_turn(&encoded)
        .expect("complete mill-sites-lure start-turn Deathrite withheld setup");
    assert_stacked_start_turn_withheld(setup);
}

fn deathrite_lure_teleport_withheld_manifest(seed: u32) -> String {
    let mut value = json!({
        "authority": {
            "contentHash": identity_hash(&json!({ "fixture": "start-turn-lure-teleport-deathrite-withheld" }))
                .expect("synthetic authority identity"),
            "mode": "synthetic",
            "revisionId": "synthetic-start-turn-lure-teleport-deathrite-withheld-v1",
        },
        "cards": {
            "north-avatar": avatar(),
            "north-pulser": minion(json!({
...
#[test]
fn rule_catalog_1225_lure_then_teleport_start_turn_trigger_withheld_during_pending_deathrite_order()
{
    let encoded = (1225..1225 + 256)
        .map(deathrite_lure_teleport_withheld_manifest)
        .find(|candidate| try_pending_deathrite_during_draw_lure_start_turn(candidate).is_some())
        .expect("bounded seed that reaches pending Deathrites during lure-teleport start-turn");
    let setup = try_pending_deathrite_during_draw_lure_start_turn(&encoded)
        .expect("complete lure-teleport start-turn Deathrite withheld setup");
    assert_stacked_start_turn_withheld(setup);
}

fn deathrite_mill_spells_life_gain_withheld_manifest(seed: u32) -> String {
    let mut value = json!({
        "authority": {
            "contentHash": identity_hash(&json!({ "fixture": "start-turn-mill-spells-life-gain-deathrite-withheld" }))
                .expect("synthetic authority identity"),
            "mode": "synthetic",
            "revisionId": "synthetic-start-turn-mill-spells-life-gain-deathrite-withheld-v1",
        },
        "cards": {
            "north-avatar": avatar(),
            "north-draw-card": minion(json!({})),
...
#[test]
fn rule_catalog_1226_mill_spells_then_life_gain_start_turn_trigger_withheld_during_pending_deathrite_order()
 {
    let encoded = (1226..1226 + 256)
        .map(deathrite_mill_spells_life_gain_withheld_manifest)
        .find(|candidate| try_pending_deathrite_during_draw_mill_start_turn(candidate).is_some())
        .expect(
            "bounded seed that reaches pending Deathrites during mill-spells-life-gain start-turn",
        );
    let setup = try_pending_deathrite_during_draw_mill_start_turn(&encoded)
        .expect("complete mill-spells-life-gain start-turn Deathrite withheld setup");
    assert_stacked_start_turn_withheld(setup);
}

fn deathrite_lure_life_loss_withheld_manifest(seed: u32) -> String {
    let mut value = json!({
        "authority": {
            "contentHash": identity_hash(&json!({ "fixture": "start-turn-lure-life-loss-deathrite-withheld" }))
                .expect("synthetic authority identity"),
            "mode": "synthetic",
            "revisionId": "synthetic-start-turn-lure-life-loss-deathrite-withheld-v1",
        },
        "cards": {
            "north-avatar": avatar(),
            "north-pulser": minion(json!({
...
#[test]
fn rule_catalog_1227_lure_then_life_loss_start_turn_trigger_withheld_during_pending_deathrite_order()
 {
    let encoded = (1227..1227 + 256)
        .map(deathrite_lure_life_loss_withheld_manifest)
        .find(|candidate| try_pending_deathrite_during_draw_lure_start_turn(candidate).is_some())
        .expect("bounded seed that reaches pending Deathrites during lure-life-loss start-turn");
    let setup = try_pending_deathrite_during_draw_lure_start_turn(&encoded)
        .expect("complete lure-life-loss start-turn Deathrite withheld setup");
    assert_stacked_start_turn_withheld(setup);
}

fn deathrite_draw_spells_here_damage_withheld_manifest(seed: u32) -> String {
    let mut value = json!({
        "authority": {
            "contentHash": identity_hash(&json!({ "fixture": "start-turn-draw-spells-here-damage-deathrite-withheld" }))
                .expect("synthetic authority identity"),
            "mode": "synthetic",
            "revisionId": "synthetic-start-turn-draw-spells-here-damage-deathrite-withheld-v1",
        },
        "cards": {
            "north-avatar": avatar(),
            "north-draw-card": minion(json!({})),
...
#[test]
fn rule_catalog_1233_draw_spells_then_here_damage_start_turn_trigger_withheld_during_pending_deathrite_order()
 {
    let encoded = (1233..1233 + 256)
        .map(deathrite_draw_spells_here_damage_withheld_manifest)
        .find(|candidate| {
            try_pending_deathrite_during_draw_sites_here_damage_start_turn(candidate).is_some()
        })
        .expect(
            "bounded seed that reaches pending Deathrites during draw-spells-here-damage start-turn",
        );
    let setup = try_pending_deathrite_during_draw_sites_here_damage_start_turn(&encoded)
        .expect("complete draw-spells-here-damage start-turn Deathrite withheld setup");
    assert_stacked_start_turn_withheld(setup);
}

fn deathrite_mill_spells_teleport_withheld_manifest(seed: u32) -> String {
    let mut value = json!({
        "authority": {
            "contentHash": identity_hash(&json!({ "fixture": "start-turn-mill-spells-teleport-deathrite-withheld" }))
                .expect("synthetic authority identity"),
            "mode": "synthetic",
            "revisionId": "synthetic-start-turn-mill-spells-teleport-deathrite-withheld-v1",
        },
        "cards": {
            "north-avatar": avatar(),
            "north-draw-card": minion(json!({})),
...
#[test]
fn rule_catalog_1234_mill_spells_then_teleport_start_turn_trigger_withheld_during_pending_deathrite_order()
 {
    let encoded = (1234..1234 + 256)
        .map(deathrite_mill_spells_teleport_withheld_manifest)
        .find(|candidate| try_pending_deathrite_during_draw_mill_start_turn(candidate).is_some())
        .expect(
            "bounded seed that reaches pending Deathrites during mill-spells-teleport start-turn",
        );
    let setup = try_pending_deathrite_during_draw_mill_start_turn(&encoded)
        .expect("complete mill-spells-teleport start-turn Deathrite withheld setup");
    assert_stacked_start_turn_withheld(setup);
}

fn deathrite_mill_sites_teleport_withheld_manifest(seed: u32) -> String {
    let mut value = json!({
        "authority": {
            "contentHash": identity_hash(&json!({ "fixture": "start-turn-mill-sites-teleport-deathrite-withheld" }))
                .expect("synthetic authority identity"),
            "mode": "synthetic",
            "revisionId": "synthetic-start-turn-mill-sites-teleport-deathrite-withheld-v1",
        },
        "cards": {
            "north-avatar": avatar(),
            "north-pulser": minion(json!({
...
#[test]
fn rule_catalog_1235_mill_sites_then_teleport_start_turn_trigger_withheld_during_pending_deathrite_order()
 {
    let encoded = (1235..1235 + 256)
        .map(deathrite_mill_sites_teleport_withheld_manifest)
        .find(|candidate| try_pending_deathrite_during_draw_mill_start_turn(candidate).is_some())
        .expect(
            "bounded seed that reaches pending Deathrites during mill-sites-teleport start-turn",
        );
    let setup = try_pending_deathrite_during_draw_mill_start_turn(&encoded)
        .expect("complete mill-sites-teleport start-turn Deathrite withheld setup");
    assert_stacked_start_turn_withheld(setup);
}

fn deathrite_mill_spells_life_loss_withheld_manifest(seed: u32) -> String {
    let mut value = json!({
        "authority": {
            "contentHash": identity_hash(&json!({ "fixture": "start-turn-mill-spells-life-loss-deathrite-withheld" }))
                .expect("synthetic authority identity"),
            "mode": "synthetic",
            "revisionId": "synthetic-start-turn-mill-spells-life-loss-deathrite-withheld-v1",
        },
        "cards": {
            "north-avatar": avatar(),
            "north-draw-card": minion(json!({})),
...
#[test]
fn rule_catalog_1236_mill_spells_then_life_loss_start_turn_trigger_withheld_during_pending_deathrite_order()
 {
    let encoded = (1236..1236 + 256)
        .map(deathrite_mill_spells_life_loss_withheld_manifest)
        .find(|candidate| try_pending_deathrite_during_draw_mill_start_turn(candidate).is_some())
        .expect(
            "bounded seed that reaches pending Deathrites during mill-spells-life-loss start-turn",
        );
    let setup = try_pending_deathrite_during_draw_mill_start_turn(&encoded)
        .expect("complete mill-spells-life-loss start-turn Deathrite withheld setup");
    assert_stacked_start_turn_withheld(setup);
}

fn deathrite_mill_sites_life_loss_withheld_manifest(seed: u32) -> String {
    let mut value = json!({
        "authority": {
            "contentHash": identity_hash(&json!({ "fixture": "start-turn-mill-sites-life-loss-deathrite-withheld" }))
                .expect("synthetic authority identity"),
            "mode": "synthetic",
            "revisionId": "synthetic-start-turn-mill-sites-life-loss-deathrite-withheld-v1",
        },
        "cards": {
            "north-avatar": avatar(),
            "north-pulser": minion(json!({
...
#[test]
fn rule_catalog_1237_mill_sites_then_life_loss_start_turn_trigger_withheld_during_pending_deathrite_order()
 {
    let encoded = (1237..1237 + 256)
        .map(deathrite_mill_sites_life_loss_withheld_manifest)
        .find(|candidate| try_pending_deathrite_during_draw_mill_start_turn(candidate).is_some())
        .expect(
            "bounded seed that reaches pending Deathrites during mill-sites-life-loss start-turn",
        );
    let setup = try_pending_deathrite_during_draw_mill_start_turn(&encoded)
        .expect("complete mill-sites-life-loss start-turn Deathrite withheld setup");
    assert_stacked_start_turn_withheld(setup);
}

fn deathrite_mill_sites_life_gain_withheld_manifest(seed: u32) -> String {
    let mut value = json!({
        "authority": {
            "contentHash": identity_hash(&json!({ "fixture": "start-turn-mill-sites-life-gain-deathrite-withheld" }))
                .expect("synthetic authority identity"),
            "mode": "synthetic",
            "revisionId": "synthetic-start-turn-mill-sites-life-gain-deathrite-withheld-v1",
        },
        "cards": {
            "north-avatar": avatar(),
            "north-pulser": minion(json!({
...
#[test]
fn rule_catalog_1238_mill_sites_then_life_gain_start_turn_trigger_withheld_during_pending_deathrite_order()
 {
    let encoded = (1238..1238 + 256)
        .map(deathrite_mill_sites_life_gain_withheld_manifest)
        .find(|candidate| try_pending_deathrite_during_draw_mill_start_turn(candidate).is_some())
        .expect(
            "bounded seed that reaches pending Deathrites during mill-sites-life-gain start-turn",
        );
    let setup = try_pending_deathrite_during_draw_mill_start_turn(&encoded)
        .expect("complete mill-sites-life-gain start-turn Deathrite withheld setup");
    assert_stacked_start_turn_withheld(setup);
}

fn deathrite_lure_life_gain_withheld_manifest(seed: u32) -> String {
    let mut value = json!({
        "authority": {
            "contentHash": identity_hash(&json!({ "fixture": "start-turn-lure-life-gain-deathrite-withheld" }))
                .expect("synthetic authority identity"),
            "mode": "synthetic",
            "revisionId": "synthetic-start-turn-lure-life-gain-deathrite-withheld-v1",
        },
        "cards": {
            "north-avatar": avatar(),
            "north-pulser": minion(json!({
...
#[test]
fn rule_catalog_1239_lure_then_life_gain_start_turn_trigger_withheld_during_pending_deathrite_order()
 {
    let encoded = (1239..1239 + 256)
        .map(deathrite_lure_life_gain_withheld_manifest)
        .find(|candidate| try_pending_deathrite_during_draw_lure_start_turn(candidate).is_some())
        .expect("bounded seed that reaches pending Deathrites during lure-life-gain start-turn");
    let setup = try_pending_deathrite_during_draw_lure_start_turn(&encoded)
        .expect("complete lure-life-gain start-turn Deathrite withheld setup");
    assert_stacked_start_turn_withheld(setup);
}

fn deathrite_lure_here_damage_withheld_manifest(seed: u32) -> String {
    let mut value = json!({
        "authority": {
            "contentHash": identity_hash(&json!({ "fixture": "start-turn-lure-here-damage-deathrite-withheld" }))
                .expect("synthetic authority identity"),
            "mode": "synthetic",
            "revisionId": "synthetic-start-turn-lure-here-damage-deathrite-withheld-v1",
        },
        "cards": {
            "north-avatar": avatar(),
            "north-pulser": minion(json!({
...
#[test]
fn rule_catalog_1240_lure_then_here_damage_start_turn_trigger_withheld_during_pending_deathrite_order()
 {
    let encoded = (1240..1240 + 256)
        .map(deathrite_lure_here_damage_withheld_manifest)
        .find(|candidate| try_pending_deathrite_during_draw_lure_start_turn(candidate).is_some())
        .expect("bounded seed that reaches pending Deathrites during lure-here-damage start-turn");
    let setup = try_pending_deathrite_during_draw_lure_start_turn(&encoded)
        .expect("complete lure-here-damage start-turn Deathrite withheld setup");
    assert_stacked_start_turn_withheld(setup);
}

fn deathrite_teleport_life_loss_withheld_manifest(seed: u32) -> String {
    let mut value = json!({
        "authority": {
            "contentHash": identity_hash(&json!({ "fixture": "start-turn-teleport-life-loss-deathrite-withheld" }))
                .expect("synthetic authority identity"),
            "mode": "synthetic",
            "revisionId": "synthetic-start-turn-teleport-life-loss-deathrite-withheld-v1",
        },
        "cards": {
            "north-avatar": avatar(),
            "north-pulser": minion(json!({
...
#[test]
fn rule_catalog_1243_teleport_then_life_loss_start_turn_trigger_withheld_during_pending_deathrite_order()
 {
    let encoded = (1243..1243 + 256)
        .map(deathrite_teleport_life_loss_withheld_manifest)
        .find(|candidate| try_pending_deathrite_during_start_turn_teleport(candidate).is_some())
        .expect(
            "bounded seed that reaches pending Deathrites during teleport-life-loss start-turn",
        );
    let mut setup = try_pending_deathrite_during_start_turn_teleport(&encoded)
        .expect("complete teleport-life-loss start-turn Deathrite withheld setup");
    let deathrite_ids = setup.deathrite_ids.clone();
    let teleport_id = setup.teleport_id.clone();
    let session = &mut setup.session;
    assert_eq!(state(session)["phase"], "trigger-order");
...
fn deathrite_teleport_life_gain_withheld_manifest(seed: u32) -> String {
    let mut value = json!({
        "authority": {
            "contentHash": identity_hash(&json!({ "fixture": "start-turn-teleport-life-gain-deathrite-withheld" }))
                .expect("synthetic authority identity"),
            "mode": "synthetic",
            "revisionId": "synthetic-start-turn-teleport-life-gain-deathrite-withheld-v1",
        },
        "cards": {
            "north-avatar": avatar(),
            "north-pulser": minion(json!({
...
#[test]
fn rule_catalog_1244_teleport_then_life_gain_start_turn_trigger_withheld_during_pending_deathrite_order()
 {
    let encoded = (1244..1244 + 256)
        .map(deathrite_teleport_life_gain_withheld_manifest)
        .find(|candidate| try_pending_deathrite_during_start_turn_teleport(candidate).is_some())
        .expect(
            "bounded seed that reaches pending Deathrites during teleport-life-gain start-turn",
        );
    let mut setup = try_pending_deathrite_during_start_turn_teleport(&encoded)
        .expect("complete teleport-life-gain start-turn Deathrite withheld setup");
    let deathrite_ids = setup.deathrite_ids.clone();
    let teleport_id = setup.teleport_id.clone();
    let session = &mut setup.session;
    assert_eq!(state(session)["phase"], "trigger-order");
...
fn deathrite_teleport_mana_gain_withheld_manifest(seed: u32) -> String {
    let mut value = json!({
        "authority": {
            "contentHash": identity_hash(&json!({ "fixture": "start-turn-teleport-mana-gain-deathrite-withheld" }))
                .expect("synthetic authority identity"),
            "mode": "synthetic",
            "revisionId": "synthetic-start-turn-teleport-mana-gain-deathrite-withheld-v1",
        },
        "cards": {
            "north-avatar": avatar(),
            "north-pulser": minion(json!({
...
#[test]
fn rule_catalog_1245_teleport_then_mana_gain_start_turn_trigger_withheld_during_pending_deathrite_order()
 {
    let encoded = (1245..1245 + 256)
        .map(deathrite_teleport_mana_gain_withheld_manifest)
        .find(|candidate| try_pending_deathrite_during_start_turn_teleport(candidate).is_some())
        .expect(
            "bounded seed that reaches pending Deathrites during teleport-mana-gain start-turn",
        );
    let mut setup = try_pending_deathrite_during_start_turn_teleport(&encoded)
        .expect("complete teleport-mana-gain start-turn Deathrite withheld setup");
    let deathrite_ids = setup.deathrite_ids.clone();
    let teleport_id = setup.teleport_id.clone();
    let session = &mut setup.session;
    assert_eq!(state(session)["phase"], "trigger-order");
...
fn deathrite_mill_spells_mana_gain_withheld_manifest(seed: u32) -> String {
    let mut value = json!({
        "authority": {
            "contentHash": identity_hash(&json!({ "fixture": "start-turn-mill-spells-mana-gain-deathrite-withheld" }))
                .expect("synthetic authority identity"),
            "mode": "synthetic",
            "revisionId": "synthetic-start-turn-mill-spells-mana-gain-deathrite-withheld-v1",
        },
        "cards": {
            "north-avatar": avatar(),
            "north-draw-card": minion(json!({})),
...
#[test]
fn rule_catalog_1246_mill_spells_then_mana_gain_start_turn_trigger_withheld_during_pending_deathrite_order()
 {
    let encoded = (1246..1246 + 256)
        .map(deathrite_mill_spells_mana_gain_withheld_manifest)
        .find(|candidate| try_pending_deathrite_during_draw_mill_start_turn(candidate).is_some())
        .expect(
            "bounded seed that reaches pending Deathrites during mill-spells-mana-gain start-turn",
        );
    let setup = try_pending_deathrite_during_draw_mill_start_turn(&encoded)
        .expect("complete mill-spells-mana-gain start-turn Deathrite withheld setup");
    assert_stacked_start_turn_withheld(setup);
}

fn deathrite_mill_sites_mana_gain_withheld_manifest(seed: u32) -> String {
    let mut value = json!({
        "authority": {
            "contentHash": identity_hash(&json!({ "fixture": "start-turn-mill-sites-mana-gain-deathrite-withheld" }))
                .expect("synthetic authority identity"),
            "mode": "synthetic",
            "revisionId": "synthetic-start-turn-mill-sites-mana-gain-deathrite-withheld-v1",
        },
        "cards": {
            "north-avatar": avatar(),
            "north-pulser": minion(json!({
...
#[test]
fn rule_catalog_1247_mill_sites_then_mana_gain_start_turn_trigger_withheld_during_pending_deathrite_order()
 {
    let encoded = (1247..1247 + 256)
        .map(deathrite_mill_sites_mana_gain_withheld_manifest)
        .find(|candidate| try_pending_deathrite_during_draw_mill_start_turn(candidate).is_some())
        .expect(
            "bounded seed that reaches pending Deathrites during mill-sites-mana-gain start-turn",
        );
    let setup = try_pending_deathrite_during_draw_mill_start_turn(&encoded)
        .expect("complete mill-sites-mana-gain start-turn Deathrite withheld setup");
    assert_stacked_start_turn_withheld(setup);
}

fn deathrite_lure_mana_gain_withheld_manifest(seed: u32) -> String {
    let mut value = json!({
        "authority": {
            "contentHash": identity_hash(&json!({ "fixture": "start-turn-lure-mana-gain-deathrite-withheld" }))
                .expect("synthetic authority identity"),
            "mode": "synthetic",
            "revisionId": "synthetic-start-turn-lure-mana-gain-deathrite-withheld-v1",
        },
        "cards": {
            "north-avatar": avatar(),
            "north-pulser": minion(json!({
...
#[test]
fn rule_catalog_1248_lure_then_mana_gain_start_turn_trigger_withheld_during_pending_deathrite_order()
 {
    let encoded = (1248..1248 + 256)
        .map(deathrite_lure_mana_gain_withheld_manifest)
        .find(|candidate| try_pending_deathrite_during_draw_lure_start_turn(candidate).is_some())
        .expect("bounded seed that reaches pending Deathrites during lure-mana-gain start-turn");
    let setup = try_pending_deathrite_during_draw_lure_start_turn(&encoded)
        .expect("complete lure-mana-gain start-turn Deathrite withheld setup");
    assert_stacked_start_turn_withheld(setup);
}

fn deathrite_teleport_here_damage_withheld_manifest(seed: u32) -> String {
    let mut value = json!({
        "authority": {
            "contentHash": identity_hash(&json!({ "fixture": "start-turn-teleport-here-damage-deathrite-withheld" }))
                .expect("synthetic authority identity"),
            "mode": "synthetic",
            "revisionId": "synthetic-start-turn-teleport-here-damage-deathrite-withheld-v1",
        },
        "cards": {
            "north-avatar": avatar(),
            "north-pulser": minion(json!({
...
#[test]
fn rule_catalog_1250_teleport_then_here_damage_start_turn_trigger_withheld_during_pending_deathrite_order()
 {
    let encoded = (1250..1250 + 256)
        .map(deathrite_teleport_here_damage_withheld_manifest)
        .find(|candidate| try_pending_deathrite_during_start_turn_teleport(candidate).is_some())
        .expect(
            "bounded seed that reaches pending Deathrites during teleport-here-damage start-turn",
        );
    let mut setup = try_pending_deathrite_during_start_turn_teleport(&encoded)
        .expect("complete teleport-here-damage start-turn Deathrite withheld setup");
    let deathrite_ids = setup.deathrite_ids.clone();
    let teleport_id = setup.teleport_id.clone();
    let session = &mut setup.session;
    assert_eq!(state(session)["phase"], "trigger-order");
...
fn deathrite_teleport_lure_withheld_manifest(seed: u32) -> String {
    let mut value = json!({
        "authority": {
            "contentHash": identity_hash(&json!({ "fixture": "start-turn-teleport-lure-deathrite-withheld" }))
                .expect("synthetic authority identity"),
            "mode": "synthetic",
            "revisionId": "synthetic-start-turn-teleport-lure-deathrite-withheld-v1",
        },
        "cards": {
            "north-avatar": avatar(),
            "north-pulser": minion(json!({
...
#[test]
fn rule_catalog_1253_teleport_then_lure_start_turn_trigger_withheld_during_pending_deathrite_order()
{
    let encoded = (1253..1253 + 256)
        .map(deathrite_teleport_lure_withheld_manifest)
        .find(|candidate| try_pending_deathrite_during_draw_lure_start_turn(candidate).is_some())
        .expect("bounded seed that reaches pending Deathrites during teleport-lure start-turn");
    let setup = try_pending_deathrite_during_draw_lure_start_turn(&encoded)
        .expect("complete teleport-lure start-turn Deathrite withheld setup");
    assert_stacked_start_turn_withheld(setup);
}

fn deathrite_teleport_mill_spells_withheld_manifest(seed: u32) -> String {
    let mut value = json!({
        "authority": {
            "contentHash": identity_hash(&json!({ "fixture": "start-turn-teleport-mill-spells-deathrite-withheld" }))
                .expect("synthetic authority identity"),
            "mode": "synthetic",
            "revisionId": "synthetic-start-turn-teleport-mill-spells-deathrite-withheld-v1",
        },
        "cards": {
            "north-avatar": avatar(),
            "north-draw-card": minion(json!({})),
...
#[test]
fn rule_catalog_1254_teleport_then_mill_spells_start_turn_trigger_withheld_during_pending_deathrite_order()
 {
    let encoded = (1254..1254 + 256)
        .map(deathrite_teleport_mill_spells_withheld_manifest)
        .find(|candidate| try_pending_deathrite_during_draw_mill_start_turn(candidate).is_some())
        .expect(
            "bounded seed that reaches pending Deathrites during teleport-mill-spells start-turn",
        );
    let setup = try_pending_deathrite_during_draw_mill_start_turn(&encoded)
        .expect("complete teleport-mill-spells start-turn Deathrite withheld setup");
    assert_stacked_start_turn_withheld(setup);
}

fn deathrite_teleport_mill_sites_withheld_manifest(seed: u32) -> String {
    let mut value = json!({
        "authority": {
            "contentHash": identity_hash(&json!({ "fixture": "start-turn-teleport-mill-sites-deathrite-withheld" }))
                .expect("synthetic authority identity"),
            "mode": "synthetic",
            "revisionId": "synthetic-start-turn-teleport-mill-sites-deathrite-withheld-v1",
        },
        "cards": {
            "north-avatar": avatar(),
            "north-pulser": minion(json!({
...
#[test]
fn rule_catalog_1255_teleport_then_mill_sites_start_turn_trigger_withheld_during_pending_deathrite_order()
 {
    let encoded = (1255..1255 + 256)
        .map(deathrite_teleport_mill_sites_withheld_manifest)
        .find(|candidate| try_pending_deathrite_during_draw_mill_start_turn(candidate).is_some())
        .expect(
            "bounded seed that reaches pending Deathrites during teleport-mill-sites start-turn",
        );
    let setup = try_pending_deathrite_during_draw_mill_start_turn(&encoded)
        .expect("complete teleport-mill-sites start-turn Deathrite withheld setup");
    assert_stacked_start_turn_withheld(setup);
}

fn deathrite_loss_teleport_withheld_manifest(seed: u32) -> String {
    let mut value = json!({
        "authority": {
            "contentHash": identity_hash(&json!({ "fixture": "start-turn-loss-teleport-deathrite-withheld" }))
                .expect("synthetic authority identity"),
            "mode": "synthetic",
            "revisionId": "synthetic-start-turn-loss-teleport-deathrite-withheld-v1",
        },
        "cards": {
            "north-avatar": avatar(),
            "north-pulser": minion(json!({
...
#[test]
fn rule_catalog_1256_loss_then_teleport_start_turn_trigger_withheld_during_pending_deathrite_order()
{
    let encoded = (1256..1256 + 256)
        .map(deathrite_loss_teleport_withheld_manifest)
        .find(|candidate| try_pending_deathrite_during_start_turn_teleport(candidate).is_some())
        .expect("bounded seed that reaches pending Deathrites during loss-teleport start-turn");
    let mut setup = try_pending_deathrite_during_start_turn_teleport(&encoded)
        .expect("complete loss-teleport start-turn Deathrite withheld setup");
    let deathrite_ids = setup.deathrite_ids.clone();
    let teleport_id = setup.teleport_id.clone();
    let session = &mut setup.session;
    assert_eq!(state(session)["phase"], "trigger-order");
...
fn deathrite_gain_teleport_withheld_manifest(seed: u32) -> String {
    let mut value = json!({
        "authority": {
            "contentHash": identity_hash(&json!({ "fixture": "start-turn-gain-teleport-deathrite-withheld" }))
                .expect("synthetic authority identity"),
            "mode": "synthetic",
            "revisionId": "synthetic-start-turn-gain-teleport-deathrite-withheld-v1",
        },
        "cards": {
            "north-avatar": avatar(),
            "north-pulser": minion(json!({
...
#[test]
fn rule_catalog_1257_gain_then_teleport_start_turn_trigger_withheld_during_pending_deathrite_order()
{
    let encoded = (1257..1257 + 256)
        .map(deathrite_gain_teleport_withheld_manifest)
        .find(|candidate| try_pending_deathrite_during_start_turn_teleport(candidate).is_some())
        .expect("bounded seed that reaches pending Deathrites during gain-teleport start-turn");
    let mut setup = try_pending_deathrite_during_start_turn_teleport(&encoded)
        .expect("complete gain-teleport start-turn Deathrite withheld setup");
    let deathrite_ids = setup.deathrite_ids.clone();
    let teleport_id = setup.teleport_id.clone();
    let session = &mut setup.session;
    assert_eq!(state(session)["phase"], "trigger-order");
...
fn deathrite_mana_teleport_withheld_manifest(seed: u32) -> String {
    let mut value = json!({
        "authority": {
            "contentHash": identity_hash(&json!({ "fixture": "start-turn-mana-teleport-deathrite-withheld" }))
                .expect("synthetic authority identity"),
            "mode": "synthetic",
            "revisionId": "synthetic-start-turn-mana-teleport-deathrite-withheld-v1",
        },
        "cards": {
            "north-avatar": avatar(),
            "north-pulser": minion(json!({
...
#[test]
fn rule_catalog_1258_mana_then_teleport_start_turn_trigger_withheld_during_pending_deathrite_order()
{
    let encoded = (1258..1258 + 256)
        .map(deathrite_mana_teleport_withheld_manifest)
        .find(|candidate| try_pending_deathrite_during_start_turn_teleport(candidate).is_some())
        .expect("bounded seed that reaches pending Deathrites during mana-teleport start-turn");
    let mut setup = try_pending_deathrite_during_start_turn_teleport(&encoded)
        .expect("complete mana-teleport start-turn Deathrite withheld setup");
    let deathrite_ids = setup.deathrite_ids.clone();
    let teleport_id = setup.teleport_id.clone();
    let session = &mut setup.session;
    assert_eq!(state(session)["phase"], "trigger-order");
...
fn deathrite_mill_spells_here_damage_withheld_manifest(seed: u32) -> String {
    let mut value = json!({
        "authority": {
            "contentHash": identity_hash(&json!({ "fixture": "start-turn-mill-spells-here-damage-deathrite-withheld" }))
                .expect("synthetic authority identity"),
            "mode": "synthetic",
            "revisionId": "synthetic-start-turn-mill-spells-here-damage-deathrite-withheld-v1",
        },
        "cards": {
            "north-avatar": avatar(),
            "north-draw-card": minion(json!({})),
...
#[test]
fn rule_catalog_1259_mill_spells_then_here_damage_start_turn_trigger_withheld_during_pending_deathrite_order()
 {
    let encoded = (1259..1259 + 256)
        .map(deathrite_mill_spells_here_damage_withheld_manifest)
        .find(|candidate| try_pending_deathrite_during_draw_mill_start_turn(candidate).is_some())
        .expect(
            "bounded seed that reaches pending Deathrites during mill-spells-here-damage start-turn",
        );
    let setup = try_pending_deathrite_during_draw_mill_start_turn(&encoded)
        .expect("complete mill-spells-here-damage start-turn Deathrite withheld setup");
    assert_stacked_start_turn_withheld(setup);
}

fn deathrite_mill_sites_here_damage_withheld_manifest(seed: u32) -> String {
    let mut value = json!({
        "authority": {
            "contentHash": identity_hash(&json!({ "fixture": "start-turn-mill-sites-here-damage-deathrite-withheld" }))
                .expect("synthetic authority identity"),
            "mode": "synthetic",
            "revisionId": "synthetic-start-turn-mill-sites-here-damage-deathrite-withheld-v1",
        },
        "cards": {
            "north-avatar": avatar(),
            "north-pulser": minion(json!({
...
#[test]
fn rule_catalog_1260_mill_sites_then_here_damage_start_turn_trigger_withheld_during_pending_deathrite_order()
 {
    let encoded = (1260..1260 + 256)
        .map(deathrite_mill_sites_here_damage_withheld_manifest)
        .find(|candidate| try_pending_deathrite_during_draw_mill_start_turn(candidate).is_some())
        .expect(
            "bounded seed that reaches pending Deathrites during mill-sites-here-damage start-turn",
        );
    let setup = try_pending_deathrite_during_draw_mill_start_turn(&encoded)
        .expect("complete mill-sites-here-damage start-turn Deathrite withheld setup");
    assert_stacked_start_turn_withheld(setup);
}

fn deathrite_here_damage_teleport_withheld_manifest(seed: u32) -> String {
    let mut value = json!({
        "authority": {
            "contentHash": identity_hash(&json!({ "fixture": "start-turn-here-damage-teleport-deathrite-withheld" }))
                .expect("synthetic authority identity"),
            "mode": "synthetic",
            "revisionId": "synthetic-start-turn-here-damage-teleport-deathrite-withheld-v1",
        },
        "cards": {
            "north-avatar": avatar(),
            "north-pulser": minion(json!({
...
#[test]
fn rule_catalog_1263_here_damage_then_teleport_start_turn_trigger_withheld_during_pending_deathrite_order()
 {
    let encoded = (1263..1263 + 256)
        .map(deathrite_here_damage_teleport_withheld_manifest)
        .find(|candidate| try_pending_deathrite_during_start_turn_teleport(candidate).is_some())
        .expect(
            "bounded seed that reaches pending Deathrites during here-damage-teleport start-turn",
        );
    let mut setup = try_pending_deathrite_during_start_turn_teleport(&encoded)
        .expect("complete here-damage-teleport start-turn Deathrite withheld setup");
    let deathrite_ids = setup.deathrite_ids.clone();
    let teleport_id = setup.teleport_id.clone();
    let session = &mut setup.session;
    assert_eq!(state(session)["phase"], "trigger-order");
...
fn deathrite_lure_mill_spells_withheld_manifest(seed: u32) -> String {
    let mut value = json!({
        "authority": {
            "contentHash": identity_hash(&json!({ "fixture": "start-turn-lure-mill-spells-deathrite-withheld" }))
                .expect("synthetic authority identity"),
            "mode": "synthetic",
            "revisionId": "synthetic-start-turn-lure-mill-spells-deathrite-withheld-v1",
        },
        "cards": {
            "north-avatar": avatar(),
            "north-draw-card": minion(json!({})),
...
#[test]
fn rule_catalog_1264_lure_then_mill_spells_start_turn_trigger_withheld_during_pending_deathrite_order()
 {
    let encoded = (1264..1264 + 256)
        .map(deathrite_lure_mill_spells_withheld_manifest)
        .find(|candidate| try_pending_deathrite_during_draw_lure_start_turn(candidate).is_some())
        .expect("bounded seed that reaches pending Deathrites during lure-mill-spells start-turn");
    let setup = try_pending_deathrite_during_draw_lure_start_turn(&encoded)
        .expect("complete lure-mill-spells start-turn Deathrite withheld setup");
    assert_stacked_start_turn_withheld(setup);
}

fn deathrite_lure_mill_sites_withheld_manifest(seed: u32) -> String {
    let mut value = json!({
        "authority": {
            "contentHash": identity_hash(&json!({ "fixture": "start-turn-lure-mill-sites-deathrite-withheld" }))
                .expect("synthetic authority identity"),
            "mode": "synthetic",
            "revisionId": "synthetic-start-turn-lure-mill-sites-deathrite-withheld-v1",
        },
        "cards": {
            "north-avatar": avatar(),
            "north-pulser": minion(json!({
...
#[test]
fn rule_catalog_1265_lure_then_mill_sites_start_turn_trigger_withheld_during_pending_deathrite_order()
 {
    let encoded = (1265..1265 + 256)
        .map(deathrite_lure_mill_sites_withheld_manifest)
        .find(|candidate| try_pending_deathrite_during_draw_lure_start_turn(candidate).is_some())
        .expect("bounded seed that reaches pending Deathrites during lure-mill-sites start-turn");
    let setup = try_pending_deathrite_during_draw_lure_start_turn(&encoded)
        .expect("complete lure-mill-sites start-turn Deathrite withheld setup");
    assert_stacked_start_turn_withheld(setup);
}

fn deathrite_loss_lure_withheld_manifest(seed: u32) -> String {
    let mut value = json!({
        "authority": {
            "contentHash": identity_hash(&json!({ "fixture": "start-turn-loss-lure-deathrite-withheld" }))
                .expect("synthetic authority identity"),
            "mode": "synthetic",
            "revisionId": "synthetic-start-turn-loss-lure-deathrite-withheld-v1",
        },
        "cards": {
            "north-avatar": avatar(),
            "north-pulser": minion(json!({
...
#[test]
fn rule_catalog_1266_loss_then_lure_start_turn_trigger_withheld_during_pending_deathrite_order() {
    let encoded = (1266..1266 + 256)
        .map(deathrite_loss_lure_withheld_manifest)
        .find(|candidate| try_pending_deathrite_during_draw_lure_start_turn(candidate).is_some())
        .expect("bounded seed that reaches pending Deathrites during loss-lure start-turn");
    let setup = try_pending_deathrite_during_draw_lure_start_turn(&encoded)
        .expect("complete loss-lure start-turn Deathrite withheld setup");
    assert_stacked_start_turn_withheld(setup);
}

fn deathrite_gain_lure_withheld_manifest(seed: u32) -> String {
    let mut value = json!({
        "authority": {
            "contentHash": identity_hash(&json!({ "fixture": "start-turn-gain-lure-deathrite-withheld" }))
                .expect("synthetic authority identity"),
            "mode": "synthetic",
            "revisionId": "synthetic-start-turn-gain-lure-deathrite-withheld-v1",
        },
        "cards": {
            "north-avatar": avatar(),
            "north-pulser": minion(json!({
...
#[test]
fn rule_catalog_1267_gain_then_lure_start_turn_trigger_withheld_during_pending_deathrite_order() {
    let encoded = (1267..1267 + 256)
        .map(deathrite_gain_lure_withheld_manifest)
        .find(|candidate| try_pending_deathrite_during_draw_lure_start_turn(candidate).is_some())
        .expect("bounded seed that reaches pending Deathrites during gain-lure start-turn");
    let setup = try_pending_deathrite_during_draw_lure_start_turn(&encoded)
        .expect("complete gain-lure start-turn Deathrite withheld setup");
    assert_stacked_start_turn_withheld(setup);
}

fn deathrite_mana_lure_withheld_manifest(seed: u32) -> String {
    let mut value = json!({
        "authority": {
            "contentHash": identity_hash(&json!({ "fixture": "start-turn-mana-lure-deathrite-withheld" }))
                .expect("synthetic authority identity"),
            "mode": "synthetic",
            "revisionId": "synthetic-start-turn-mana-lure-deathrite-withheld-v1",
        },
        "cards": {
            "north-avatar": avatar(),
            "north-pulser": minion(json!({
...
#[test]
fn rule_catalog_1268_mana_then_lure_start_turn_trigger_withheld_during_pending_deathrite_order() {
    let encoded = (1268..1268 + 256)
        .map(deathrite_mana_lure_withheld_manifest)
        .find(|candidate| try_pending_deathrite_during_draw_lure_start_turn(candidate).is_some())
        .expect("bounded seed that reaches pending Deathrites during mana-lure start-turn");
    let setup = try_pending_deathrite_during_draw_lure_start_turn(&encoded)
        .expect("complete mana-lure start-turn Deathrite withheld setup");
    assert_stacked_start_turn_withheld(setup);
}

fn deathrite_here_damage_life_loss_withheld_manifest(seed: u32) -> String {
    let mut value = json!({
        "authority": {
            "contentHash": identity_hash(&json!({ "fixture": "start-turn-here-damage-life-loss-deathrite-withheld" }))
                .expect("synthetic authority identity"),
            "mode": "synthetic",
            "revisionId": "synthetic-start-turn-here-damage-life-loss-deathrite-withheld-v1",
        },
        "cards": {
            "north-avatar": avatar(),
            "north-pulser": minion(json!({
...
#[test]
fn rule_catalog_1269_here_damage_then_life_loss_start_turn_trigger_withheld_during_pending_deathrite_order()
 {
    let encoded = (1269..1269 + 256)
        .map(deathrite_here_damage_life_loss_withheld_manifest)
        .find(|candidate| {
            try_pending_deathrite_during_draw_sites_here_damage_start_turn(candidate).is_some()
        })
        .expect(
            "bounded seed that reaches pending Deathrites during here-damage-life-loss start-turn",
        );
    let setup = try_pending_deathrite_during_draw_sites_here_damage_start_turn(&encoded)
        .expect("complete here-damage-life-loss start-turn Deathrite withheld setup");
    assert_stacked_start_turn_withheld(setup);
}

fn deathrite_here_damage_mana_gain_withheld_manifest(seed: u32) -> String {
    let mut value = json!({
        "authority": {
            "contentHash": identity_hash(&json!({ "fixture": "start-turn-here-damage-mana-gain-deathrite-withheld" }))
                .expect("synthetic authority identity"),
            "mode": "synthetic",
            "revisionId": "synthetic-start-turn-here-damage-mana-gain-deathrite-withheld-v1",
        },
        "cards": {
            "north-avatar": avatar(),
            "north-pulser": minion(json!({
...
#[test]
fn rule_catalog_1270_here_damage_then_mana_gain_start_turn_trigger_withheld_during_pending_deathrite_order()
 {
    let encoded = (1270..1270 + 256)
        .map(deathrite_here_damage_mana_gain_withheld_manifest)
        .find(|candidate| {
            try_pending_deathrite_during_draw_sites_here_damage_start_turn(candidate).is_some()
        })
        .expect(
            "bounded seed that reaches pending Deathrites during here-damage-mana-gain start-turn",
        );
    let setup = try_pending_deathrite_during_draw_sites_here_damage_start_turn(&encoded)
        .expect("complete here-damage-mana-gain start-turn Deathrite withheld setup");
    assert_stacked_start_turn_withheld(setup);
}

fn deathrite_here_damage_life_gain_withheld_manifest(seed: u32) -> String {
    let mut value = json!({
        "authority": {
            "contentHash": identity_hash(&json!({ "fixture": "start-turn-here-damage-life-gain-deathrite-withheld" }))
                .expect("synthetic authority identity"),
            "mode": "synthetic",
            "revisionId": "synthetic-start-turn-here-damage-life-gain-deathrite-withheld-v1",
        },
        "cards": {
            "north-avatar": avatar(),
            "north-pulser": minion(json!({
...
#[test]
fn rule_catalog_1273_here_damage_then_life_gain_start_turn_trigger_withheld_during_pending_deathrite_order()
 {
    let encoded = (1273..1273 + 256)
        .map(deathrite_here_damage_life_gain_withheld_manifest)
        .find(|candidate| {
            try_pending_deathrite_during_draw_sites_here_damage_start_turn(candidate).is_some()
        })
        .expect(
            "bounded seed that reaches pending Deathrites during here-damage-life-gain start-turn",
        );
    let setup = try_pending_deathrite_during_draw_sites_here_damage_start_turn(&encoded)
        .expect("complete here-damage-life-gain start-turn Deathrite withheld setup");
    assert_stacked_start_turn_withheld(setup);
}

fn deathrite_here_damage_lure_withheld_manifest(seed: u32) -> String {
    let mut value = json!({
        "authority": {
            "contentHash": identity_hash(&json!({ "fixture": "start-turn-here-damage-lure-deathrite-withheld" }))
                .expect("synthetic authority identity"),
            "mode": "synthetic",
            "revisionId": "synthetic-start-turn-here-damage-lure-deathrite-withheld-v1",
        },
        "cards": {
            "north-avatar": avatar(),
            "north-pulser": minion(json!({
...
#[test]
fn rule_catalog_1274_here_damage_then_lure_start_turn_trigger_withheld_during_pending_deathrite_order()
 {
    let encoded = (1274..1274 + 256)
        .map(deathrite_here_damage_lure_withheld_manifest)
        .find(|candidate| try_pending_deathrite_during_draw_lure_start_turn(candidate).is_some())
        .expect("bounded seed that reaches pending Deathrites during here-damage-lure start-turn");
    let setup = try_pending_deathrite_during_draw_lure_start_turn(&encoded)
        .expect("complete here-damage-lure start-turn Deathrite withheld setup");
    assert_stacked_start_turn_withheld(setup);
}

fn deathrite_gain_mill_spells_withheld_manifest(seed: u32) -> String {
    let mut value = json!({
        "authority": {
            "contentHash": identity_hash(&json!({ "fixture": "start-turn-gain-mill-spells-deathrite-withheld" }))
                .expect("synthetic authority identity"),
            "mode": "synthetic",
            "revisionId": "synthetic-start-turn-gain-mill-spells-deathrite-withheld-v1",
        },
        "cards": {
            "north-avatar": avatar(),
            "north-mill-card": minion(json!({})),
...
#[test]
fn rule_catalog_1275_gain_then_mill_spells_start_turn_trigger_withheld_during_pending_deathrite_order()
 {
    let encoded = (1275..1275 + 256)
        .map(deathrite_gain_mill_spells_withheld_manifest)
        .find(|candidate| try_pending_deathrite_during_draw_mill_start_turn(candidate).is_some())
        .expect("bounded seed that reaches pending Deathrites during gain-mill-spells start-turn");
    let setup = try_pending_deathrite_during_draw_mill_start_turn(&encoded)
        .expect("complete gain-mill-spells start-turn Deathrite withheld setup");
    assert_stacked_start_turn_withheld(setup);
}

fn deathrite_gain_mill_sites_withheld_manifest(seed: u32) -> String {
    let mut value = json!({
        "authority": {
            "contentHash": identity_hash(&json!({ "fixture": "start-turn-gain-mill-sites-deathrite-withheld" }))
                .expect("synthetic authority identity"),
            "mode": "synthetic",
            "revisionId": "synthetic-start-turn-gain-mill-sites-deathrite-withheld-v1",
        },
        "cards": {
            "north-avatar": avatar(),
            "north-pulser": minion(json!({
...
#[test]
fn rule_catalog_1276_gain_then_mill_sites_start_turn_trigger_withheld_during_pending_deathrite_order()
 {
    let encoded = (1276..1276 + 256)
        .map(deathrite_gain_mill_sites_withheld_manifest)
        .find(|candidate| try_pending_deathrite_during_draw_mill_start_turn(candidate).is_some())
        .expect("bounded seed that reaches pending Deathrites during gain-mill-sites start-turn");
    let setup = try_pending_deathrite_during_draw_mill_start_turn(&encoded)
        .expect("complete gain-mill-sites start-turn Deathrite withheld setup");
    assert_stacked_start_turn_withheld(setup);
}

fn deathrite_loss_mill_spells_withheld_manifest(seed: u32) -> String {
    let mut value = json!({
        "authority": {
            "contentHash": identity_hash(&json!({ "fixture": "start-turn-loss-mill-spells-deathrite-withheld" }))
                .expect("synthetic authority identity"),
            "mode": "synthetic",
            "revisionId": "synthetic-start-turn-loss-mill-spells-deathrite-withheld-v1",
        },
        "cards": {
            "north-avatar": avatar(),
            "north-mill-card": minion(json!({})),
...
#[test]
fn rule_catalog_1277_loss_then_mill_spells_start_turn_trigger_withheld_during_pending_deathrite_order()
 {
    let encoded = (1277..1277 + 256)
        .map(deathrite_loss_mill_spells_withheld_manifest)
        .find(|candidate| try_pending_deathrite_during_draw_mill_start_turn(candidate).is_some())
        .expect("bounded seed that reaches pending Deathrites during loss-mill-spells start-turn");
    let setup = try_pending_deathrite_during_draw_mill_start_turn(&encoded)
        .expect("complete loss-mill-spells start-turn Deathrite withheld setup");
    assert_stacked_start_turn_withheld(setup);
}

fn deathrite_loss_mill_sites_withheld_manifest(seed: u32) -> String {
    let mut value = json!({
        "authority": {
            "contentHash": identity_hash(&json!({ "fixture": "start-turn-loss-mill-sites-deathrite-withheld" }))
                .expect("synthetic authority identity"),
            "mode": "synthetic",
            "revisionId": "synthetic-start-turn-loss-mill-sites-deathrite-withheld-v1",
        },
        "cards": {
            "north-avatar": avatar(),
            "north-pulser": minion(json!({
...
#[test]
fn rule_catalog_1278_loss_then_mill_sites_start_turn_trigger_withheld_during_pending_deathrite_order()
 {
    let encoded = (1278..1278 + 256)
        .map(deathrite_loss_mill_sites_withheld_manifest)
        .find(|candidate| try_pending_deathrite_during_draw_mill_start_turn(candidate).is_some())
        .expect("bounded seed that reaches pending Deathrites during loss-mill-sites start-turn");
    let setup = try_pending_deathrite_during_draw_mill_start_turn(&encoded)
        .expect("complete loss-mill-sites start-turn Deathrite withheld setup");
    assert_stacked_start_turn_withheld(setup);
}

fn deathrite_mana_life_loss_withheld_manifest(seed: u32) -> String {
    let mut value = json!({
        "authority": {
            "contentHash": identity_hash(&json!({ "fixture": "start-turn-mana-life-loss-deathrite-withheld" }))
                .expect("synthetic authority identity"),
            "mode": "synthetic",
            "revisionId": "synthetic-start-turn-mana-life-loss-deathrite-withheld-v1",
        },
        "cards": {
            "north-avatar": avatar(),
            "north-pulser": minion(json!({
...
#[test]
fn rule_catalog_1279_mana_then_life_loss_start_turn_trigger_withheld_during_pending_deathrite_order()
 {
    let encoded = (1279..1279 + 256)
        .map(deathrite_mana_life_loss_withheld_manifest)
        .find(|candidate| try_pending_deathrite_during_draw_mill_start_turn(candidate).is_some())
        .expect("bounded seed that reaches pending Deathrites during mana-life-loss start-turn");
    let setup = try_pending_deathrite_during_draw_mill_start_turn(&encoded)
        .expect("complete mana-life-loss start-turn Deathrite withheld setup");
    assert_stacked_start_turn_withheld(setup);
}

fn deathrite_mana_life_gain_withheld_manifest(seed: u32) -> String {
    let mut value = json!({
        "authority": {
            "contentHash": identity_hash(&json!({ "fixture": "start-turn-mana-life-gain-deathrite-withheld" }))
                .expect("synthetic authority identity"),
            "mode": "synthetic",
            "revisionId": "synthetic-start-turn-mana-life-gain-deathrite-withheld-v1",
        },
        "cards": {
            "north-avatar": avatar(),
            "north-pulser": minion(json!({
...
#[test]
fn rule_catalog_1280_mana_then_life_gain_start_turn_trigger_withheld_during_pending_deathrite_order()
 {
    let encoded = (1280..1280 + 256)
        .map(deathrite_mana_life_gain_withheld_manifest)
        .find(|candidate| try_pending_deathrite_during_draw_mill_start_turn(candidate).is_some())
        .expect("bounded seed that reaches pending Deathrites during mana-life-gain start-turn");
    let setup = try_pending_deathrite_during_draw_mill_start_turn(&encoded)
        .expect("complete mana-life-gain start-turn Deathrite withheld setup");
    assert_stacked_start_turn_withheld(setup);
}

fn deathrite_here_damage_mill_spells_withheld_manifest(seed: u32) -> String {
    let mut value = json!({
        "authority": {
            "contentHash": identity_hash(&json!({ "fixture": "start-turn-here-damage-mill-spells-deathrite-withheld" }))
                .expect("synthetic authority identity"),
            "mode": "synthetic",
            "revisionId": "synthetic-start-turn-here-damage-mill-spells-deathrite-withheld-v1",
        },
        "cards": {
            "north-avatar": avatar(),
            "north-mill-card": minion(json!({})),
...
#[test]
fn rule_catalog_1283_here_damage_then_mill_spells_start_turn_trigger_withheld_during_pending_deathrite_order()
 {
    let encoded = (1283..1283 + 256)
        .map(deathrite_here_damage_mill_spells_withheld_manifest)
        .find(|candidate| try_pending_deathrite_during_draw_mill_start_turn(candidate).is_some())
        .expect(
            "bounded seed that reaches pending Deathrites during here-damage-mill-spells start-turn",
        );
    let setup = try_pending_deathrite_during_draw_mill_start_turn(&encoded)
        .expect("complete here-damage-mill-spells start-turn Deathrite withheld setup");
    assert_stacked_start_turn_withheld(setup);
}

fn deathrite_here_damage_mill_sites_withheld_manifest(seed: u32) -> String {
    let mut value = json!({
        "authority": {
            "contentHash": identity_hash(&json!({ "fixture": "start-turn-here-damage-mill-sites-deathrite-withheld" }))
                .expect("synthetic authority identity"),
            "mode": "synthetic",
            "revisionId": "synthetic-start-turn-here-damage-mill-sites-deathrite-withheld-v1",
        },
        "cards": {
            "north-avatar": avatar(),
            "north-pulser": minion(json!({
...
#[test]
fn rule_catalog_1284_here_damage_then_mill_sites_start_turn_trigger_withheld_during_pending_deathrite_order()
 {
    let encoded = (1284..1284 + 256)
        .map(deathrite_here_damage_mill_sites_withheld_manifest)
        .find(|candidate| try_pending_deathrite_during_draw_mill_start_turn(candidate).is_some())
        .expect(
            "bounded seed that reaches pending Deathrites during here-damage-mill-sites start-turn",
        );
    let setup = try_pending_deathrite_during_draw_mill_start_turn(&encoded)
        .expect("complete here-damage-mill-sites start-turn Deathrite withheld setup");
    assert_stacked_start_turn_withheld(setup);
}

fn deathrite_draw_spells_mill_spells_withheld_manifest(seed: u32) -> String {
    let mut value = json!({
        "authority": {
            "contentHash": identity_hash(&json!({ "fixture": "start-turn-draw-spells-mill-spells-deathrite-withheld" }))
                .expect("synthetic authority identity"),
            "mode": "synthetic",
            "revisionId": "synthetic-start-turn-draw-spells-mill-spells-deathrite-withheld-v1",
        },
        "cards": {
            "north-avatar": avatar(),
            "north-draw-card": minion(json!({})),
...
#[test]
fn rule_catalog_1285_draw_spells_then_mill_spells_start_turn_trigger_withheld_during_pending_deathrite_order()
 {
    let encoded = (1285..1285 + 256)
        .map(deathrite_draw_spells_mill_spells_withheld_manifest)
        .find(|candidate| try_pending_deathrite_during_draw_mill_start_turn(candidate).is_some())
        .expect(
            "bounded seed that reaches pending Deathrites during draw-spells-mill-spells start-turn",
        );
    let setup = try_pending_deathrite_during_draw_mill_start_turn(&encoded)
        .expect("complete draw-spells-mill-spells start-turn Deathrite withheld setup");
    assert_stacked_start_turn_withheld(setup);
}

fn deathrite_mana_mill_spells_withheld_manifest(seed: u32) -> String {
    let mut value = json!({
        "authority": {
            "contentHash": identity_hash(&json!({ "fixture": "start-turn-mana-mill-spells-deathrite-withheld" }))
                .expect("synthetic authority identity"),
            "mode": "synthetic",
            "revisionId": "synthetic-start-turn-mana-mill-spells-deathrite-withheld-v1",
        },
        "cards": {
            "north-avatar": avatar(),
            "north-mill-card": minion(json!({})),
...
#[test]
fn rule_catalog_1286_mana_then_mill_spells_start_turn_trigger_withheld_during_pending_deathrite_order()
 {
    let encoded = (1286..1286 + 256)
        .map(deathrite_mana_mill_spells_withheld_manifest)
        .find(|candidate| try_pending_deathrite_during_draw_mill_start_turn(candidate).is_some())
        .expect("bounded seed that reaches pending Deathrites during mana-mill-spells start-turn");
    let setup = try_pending_deathrite_during_draw_mill_start_turn(&encoded)
        .expect("complete mana-mill-spells start-turn Deathrite withheld setup");
    assert_stacked_start_turn_withheld(setup);
}

fn deathrite_mana_mill_sites_withheld_manifest(seed: u32) -> String {
    let mut value = json!({
        "authority": {
            "contentHash": identity_hash(&json!({ "fixture": "start-turn-mana-mill-sites-deathrite-withheld" }))
                .expect("synthetic authority identity"),
            "mode": "synthetic",
            "revisionId": "synthetic-start-turn-mana-mill-sites-deathrite-withheld-v1",
        },
        "cards": {
            "north-avatar": avatar(),
            "north-pulser": minion(json!({
...
#[test]
fn rule_catalog_1287_mana_then_mill_sites_start_turn_trigger_withheld_during_pending_deathrite_order()
 {
    let encoded = (1287..1287 + 256)
        .map(deathrite_mana_mill_sites_withheld_manifest)
        .find(|candidate| try_pending_deathrite_during_draw_mill_start_turn(candidate).is_some())
        .expect("bounded seed that reaches pending Deathrites during mana-mill-sites start-turn");
    let setup = try_pending_deathrite_during_draw_mill_start_turn(&encoded)
        .expect("complete mana-mill-sites start-turn Deathrite withheld setup");
    assert_stacked_start_turn_withheld(setup);
}

fn deathrite_gain_mana_withheld_manifest(seed: u32) -> String {
    let mut value = json!({
        "authority": {
            "contentHash": identity_hash(&json!({ "fixture": "start-turn-gain-mana-deathrite-withheld" }))
                .expect("synthetic authority identity"),
            "mode": "synthetic",
            "revisionId": "synthetic-start-turn-gain-mana-deathrite-withheld-v1",
        },
        "cards": {
            "north-avatar": avatar(),
            "north-pulser": minion(json!({
...
#[test]
fn rule_catalog_1288_gain_then_mana_start_turn_trigger_withheld_during_pending_deathrite_order() {
    let encoded = (1288..1288 + 256)
        .map(deathrite_gain_mana_withheld_manifest)
        .find(|candidate| try_pending_deathrite_during_draw_mill_start_turn(candidate).is_some())
        .expect("bounded seed that reaches pending Deathrites during gain-mana start-turn");
    let setup = try_pending_deathrite_during_draw_mill_start_turn(&encoded)
        .expect("complete gain-mana start-turn Deathrite withheld setup");
    assert_stacked_start_turn_withheld(setup);
}

fn deathrite_draw_spells_draw_sites_withheld_manifest(seed: u32) -> String {
    let mut value = json!({
        "authority": {
            "contentHash": identity_hash(&json!({ "fixture": "start-turn-draw-spells-draw-sites-deathrite-withheld" }))
                .expect("synthetic authority identity"),
            "mode": "synthetic",
            "revisionId": "synthetic-start-turn-draw-spells-draw-sites-deathrite-withheld-v1",
        },
        "cards": {
            "north-avatar": avatar(),
            "north-draw-card": minion(json!({})),
...
#[test]
fn rule_catalog_1289_draw_spells_then_draw_sites_start_turn_trigger_withheld_during_pending_deathrite_order()
 {
    let encoded = (1289..1289 + 256)
        .map(deathrite_draw_spells_draw_sites_withheld_manifest)
        .find(|candidate| try_pending_deathrite_during_draw_mill_start_turn(candidate).is_some())
        .expect(
            "bounded seed that reaches pending Deathrites during draw-spells-draw-sites start-turn",
        );
    let setup = try_pending_deathrite_during_draw_mill_start_turn(&encoded)
        .expect("complete draw-spells-draw-sites start-turn Deathrite withheld setup");
    assert_stacked_start_turn_withheld(setup);
}

fn deathrite_draw_sites_mill_sites_withheld_manifest(seed: u32) -> String {
    let mut value = json!({
        "authority": {
            "contentHash": identity_hash(&json!({ "fixture": "start-turn-draw-sites-mill-sites-deathrite-withheld" }))
                .expect("synthetic authority identity"),
            "mode": "synthetic",
            "revisionId": "synthetic-start-turn-draw-sites-mill-sites-deathrite-withheld-v1",
        },
        "cards": {
            "north-avatar": avatar(),
            "north-pulser": minion(json!({
...
#[test]
fn rule_catalog_1290_draw_sites_then_mill_sites_start_turn_trigger_withheld_during_pending_deathrite_order()
 {
    let encoded = (1290..1290 + 256)
        .map(deathrite_draw_sites_mill_sites_withheld_manifest)
        .find(|candidate| try_pending_deathrite_during_draw_mill_start_turn(candidate).is_some())
        .expect(
            "bounded seed that reaches pending Deathrites during draw-sites-mill-sites start-turn",
        );
    let setup = try_pending_deathrite_during_draw_mill_start_turn(&encoded)
...
fn deathrite_mill_spells_draw_sites_withheld_manifest(seed: u32) -> String {
    let mut value = json!({
        "authority": {
            "contentHash": identity_hash(&json!({ "fixture": "start-turn-mill-spells-draw-sites-deathrite-withheld" }))
                .expect("synthetic authority identity"),
            "mode": "synthetic",
            "revisionId": "synthetic-start-turn-mill-spells-draw-sites-deathrite-withheld-v1",
        },
        "cards": {
            "north-avatar": avatar(),
            "north-mill-card": minion(json!({})),
...
#[test]
fn rule_catalog_1303_mill_spells_then_draw_sites_start_turn_trigger_withheld_during_pending_deathrite_order()
 {
    let encoded = (1303..1303 + 256)
        .map(deathrite_mill_spells_draw_sites_withheld_manifest)
        .find(|candidate| try_pending_deathrite_during_draw_mill_start_turn(candidate).is_some())
        .expect(
            "bounded seed that reaches pending Deathrites during mill-spells-draw-sites start-turn",
        );
    let setup = try_pending_deathrite_during_draw_mill_start_turn(&encoded)
        .expect("complete mill-spells-draw-sites start-turn Deathrite withheld setup");
    assert_stacked_start_turn_withheld(setup);
}

fn deathrite_mill_sites_draw_spells_withheld_manifest(seed: u32) -> String {
    let mut value = json!({
        "authority": {
            "contentHash": identity_hash(&json!({ "fixture": "start-turn-mill-sites-draw-spells-deathrite-withheld" }))
                .expect("synthetic authority identity"),
            "mode": "synthetic",
            "revisionId": "synthetic-start-turn-mill-sites-draw-spells-deathrite-withheld-v1",
        },
        "cards": {
            "north-avatar": avatar(),
            "north-draw-card": minion(json!({})),
...
#[test]
fn rule_catalog_1304_mill_sites_then_draw_spells_start_turn_trigger_withheld_during_pending_deathrite_order()
 {
    let encoded = (1304..1304 + 256)
        .map(deathrite_mill_sites_draw_spells_withheld_manifest)
        .find(|candidate| try_pending_deathrite_during_draw_mill_start_turn(candidate).is_some())
        .expect(
            "bounded seed that reaches pending Deathrites during mill-sites-draw-spells start-turn",
        );
    let setup = try_pending_deathrite_during_draw_mill_start_turn(&encoded)
        .expect("complete mill-sites-draw-spells start-turn Deathrite withheld setup");
    assert_stacked_start_turn_withheld(setup);
}

fn deathrite_lure_draw_spells_withheld_manifest(seed: u32) -> String {
    let mut value = json!({
        "authority": {
            "contentHash": identity_hash(&json!({ "fixture": "start-turn-lure-draw-spells-deathrite-withheld" }))
                .expect("synthetic authority identity"),
            "mode": "synthetic",
            "revisionId": "synthetic-start-turn-lure-draw-spells-deathrite-withheld-v1",
        },
        "cards": {
            "north-avatar": avatar(),
            "north-draw-card": minion(json!({})),
...
#[test]
fn rule_catalog_1305_lure_then_draw_spells_start_turn_trigger_withheld_during_pending_deathrite_order()
 {
    let encoded = (1305..1305 + 256)
        .map(deathrite_lure_draw_spells_withheld_manifest)
        .find(|candidate| try_pending_deathrite_during_draw_lure_start_turn(candidate).is_some())
        .expect("bounded seed that reaches pending Deathrites during lure-draw-spells start-turn");
    let setup = try_pending_deathrite_during_draw_lure_start_turn(&encoded)
        .expect("complete lure-draw-spells start-turn Deathrite withheld setup");
    assert_stacked_start_turn_withheld(setup);
}

fn deathrite_lure_draw_sites_withheld_manifest(seed: u32) -> String {
    let mut value = json!({
        "authority": {
            "contentHash": identity_hash(&json!({ "fixture": "start-turn-lure-draw-sites-deathrite-withheld" }))
                .expect("synthetic authority identity"),
            "mode": "synthetic",
            "revisionId": "synthetic-start-turn-lure-draw-sites-deathrite-withheld-v1",
        },
        "cards": {
            "north-avatar": avatar(),
            "north-pulser": minion(json!({
...
#[test]
fn rule_catalog_1306_lure_then_draw_sites_start_turn_trigger_withheld_during_pending_deathrite_order()
 {
    let encoded = (1306..1306 + 256)
        .map(deathrite_lure_draw_sites_withheld_manifest)
        .find(|candidate| try_pending_deathrite_during_draw_lure_start_turn(candidate).is_some())
        .expect("bounded seed that reaches pending Deathrites during lure-draw-sites start-turn");
    let setup = try_pending_deathrite_during_draw_lure_start_turn(&encoded)
        .expect("complete lure-draw-sites start-turn Deathrite withheld setup");
    assert_stacked_start_turn_withheld(setup);
}

fn deathrite_here_draw_spells_withheld_manifest(seed: u32) -> String {
    let mut value = json!({
        "authority": {
            "contentHash": identity_hash(&json!({ "fixture": "start-turn-here-draw-spells-deathrite-withheld" }))
                .expect("synthetic authority identity"),
            "mode": "synthetic",
            "revisionId": "synthetic-start-turn-here-draw-spells-deathrite-withheld-v1",
        },
        "cards": {
            "north-avatar": avatar(),
            "north-draw-card": minion(json!({})),
...
#[test]
fn rule_catalog_1307_here_damage_then_draw_spells_start_turn_trigger_withheld_during_pending_deathrite_order()
 {
    let encoded = (1307..1307 + 256)
        .map(deathrite_here_draw_spells_withheld_manifest)
        .find(|candidate| {
            try_pending_deathrite_during_draw_sites_here_damage_start_turn(candidate).is_some()
        })
        .expect("bounded seed that reaches pending Deathrites during here-draw-spells start-turn");
    let setup = try_pending_deathrite_during_draw_sites_here_damage_start_turn(&encoded)
        .expect("complete here-draw-spells start-turn Deathrite withheld setup");
    assert_stacked_start_turn_withheld(setup);
}

fn deathrite_here_draw_sites_withheld_manifest(seed: u32) -> String {
    let mut value = json!({
        "authority": {
            "contentHash": identity_hash(&json!({ "fixture": "start-turn-here-draw-sites-deathrite-withheld" }))
                .expect("synthetic authority identity"),
            "mode": "synthetic",
            "revisionId": "synthetic-start-turn-here-draw-sites-deathrite-withheld-v1",
        },
        "cards": {
            "north-avatar": avatar(),
            "north-pulser": minion(json!({
...
#[test]
fn rule_catalog_1308_here_damage_then_draw_sites_start_turn_trigger_withheld_during_pending_deathrite_order()
 {
    let encoded = (1308..1308 + 256)
        .map(deathrite_here_draw_sites_withheld_manifest)
        .find(|candidate| {
            try_pending_deathrite_during_draw_sites_here_damage_start_turn(candidate).is_some()
        })
        .expect("bounded seed that reaches pending Deathrites during here-draw-sites start-turn");
    let setup = try_pending_deathrite_during_draw_sites_here_damage_start_turn(&encoded)
        .expect("complete here-draw-sites start-turn Deathrite withheld setup");
    assert_stacked_start_turn_withheld(setup);
}

fn deathrite_teleport_draw_spells_withheld_manifest(seed: u32) -> String {
    let mut value = json!({
        "authority": {
            "contentHash": identity_hash(&json!({ "fixture": "start-turn-teleport-draw-spells-deathrite-withheld" }))
                .expect("synthetic authority identity"),
            "mode": "synthetic",
            "revisionId": "synthetic-start-turn-teleport-draw-spells-deathrite-withheld-v1",
        },
        "cards": {
            "north-avatar": avatar(),
            "north-draw-card": minion(json!({})),
...
#[test]
fn rule_catalog_1309_teleport_then_draw_spells_start_turn_trigger_withheld_during_pending_deathrite_order()
 {
    let encoded = (1309..1309 + 256)
        .map(deathrite_teleport_draw_spells_withheld_manifest)
        .find(|candidate| try_pending_deathrite_during_start_turn_teleport(candidate).is_some())
        .expect(
            "bounded seed that reaches pending Deathrites during teleport-draw-spells start-turn",
        );
    let mut setup = try_pending_deathrite_during_start_turn_teleport(&encoded)
        .expect("complete teleport-draw-spells start-turn Deathrite withheld setup");
    let deathrite_ids = setup.deathrite_ids.clone();
    let teleport_id = setup.teleport_id.clone();
    let session = &mut setup.session;
    assert_eq!(state(session)["phase"], "trigger-order");
...
fn deathrite_teleport_draw_sites_withheld_manifest(seed: u32) -> String {
    let mut value = json!({
        "authority": {
            "contentHash": identity_hash(&json!({ "fixture": "start-turn-teleport-draw-sites-deathrite-withheld" }))
                .expect("synthetic authority identity"),
            "mode": "synthetic",
            "revisionId": "synthetic-start-turn-teleport-draw-sites-deathrite-withheld-v1",
        },
        "cards": {
            "north-avatar": avatar(),
            "north-pulser": minion(json!({
...
#[test]
fn rule_catalog_1310_teleport_then_draw_sites_start_turn_trigger_withheld_during_pending_deathrite_order()
 {
    let encoded = (1310..1310 + 256)
        .map(deathrite_teleport_draw_sites_withheld_manifest)
        .find(|candidate| try_pending_deathrite_during_start_turn_teleport(candidate).is_some())
        .expect(
            "bounded seed that reaches pending Deathrites during teleport-draw-sites start-turn",
        );
    let mut setup = try_pending_deathrite_during_start_turn_teleport(&encoded)
        .expect("complete teleport-draw-sites start-turn Deathrite withheld setup");
    let deathrite_ids = setup.deathrite_ids.clone();
    let teleport_id = setup.teleport_id.clone();
    let session = &mut setup.session;
    assert_eq!(state(session)["phase"], "trigger-order");
