...
#[expect(
    clippy::too_many_lines,
    reason = "one direct scenario proves continue-basic-movement stays withheld until Deathrites are ordered"
)]
fn rule_catalog_1006_continue_basic_movement_withheld_during_pending_deathrite_order() {
    let cards = json!({
        "north-avatar": avatar(),
        "north-mover": minion(json!({
            "lethal": true,
    ...
    let encoded = power_loss_manifest(
        198,
        "synthetic-ranged-movement-deathrite-withheld-v1",
        &cards,
        ["north-shooter", "north-shooter", "north-mover"],
        ["south-aura", "south-deathrite-a", "south-deathrite-b"],
        6,
    ...
    let mut session = Session::new(&encoded).expect("valid movement Deathrite withheld scenario");
    keep(&mut session);
    keep(&mut session);
    accept_where(&mut session, |descriptor| {
        descriptor["kind"] == "play-site" && descriptor["cell"] == "C4"
    ...
    assert!(
        session
            ...
            .iter()
            .all(|action| action.descriptor["kind"] != "continue-basic-movement")
    );
    assert!(
        session
            .legal_actions()
    ...
    assert_eq!(resumed["decisionSeat"], "north");
    assert_eq!(
        resumed["pendingBasicMovement"]["sourceInstanceId"],
        mover_id
    );
    ...
    let (_, continued) = accept_where(&mut session, |descriptor| {
        descriptor["kind"] == "continue-basic-movement" && descriptor["unitInstanceId"] == mover_id
    });
    assert_eq!(event_types(&continued), ["basic-movement-continued"]);
    assert_eq!(state(&session)["pendingBasicMovement"]["pathIndex"], 2);
    let (_, finished) = accept_where(&mut session, |descriptor| {
        descriptor["kind"] == "continue-basic-movement" && descriptor["unitInstanceId"] == mover_id
    });
    assert_eq!(event_types(&finished), ["move-and-attack-activated"]);
...
fn deathrite_shoot_manifest(seed: u32) -> String {
    let fixture = "ranged-shoot-deathrite-withheld";
    let cards = json!({
        "north-avatar": avatar(),
        "north-rain": rain(),
        "north-shooter": minion(json!({ "ranged": true })),
...
struct PendingDeathriteShootSetup {
    deathrite_ids: [String; 2],
    session: Session,
    shooter_id: String,
}

fn try_pending_deathrite_with_ready_shooter(encoded: &str) -> Option<PendingDeathriteShootSetup> {
    ...
    if ranged_actions(&session, &shooter_id).is_empty() {
        return None;
    }
    try_accept_where(&mut session, |descriptor| {
    ...
    Some(PendingDeathriteShootSetup {
        deathrite_ids,
        session,
        shooter_id,
    })
...
fn deathrite_shoot_seed_with(start: u32) -> String {
    (start..start + 2048)
        .map(deathrite_shoot_manifest)
        .find(|candidate| try_pending_deathrite_with_ready_shooter(candidate).is_some())
        .expect(
            "bounded seed that reaches pending Deathrites with a ready Ranged minion on the board",
        )
}

#[test]
fn rule_catalog_1119_shoot_projectile_withheld_during_pending_deathrite_order() {
    let encoded = deathrite_shoot_seed_with(1119);
    let mut setup = try_pending_deathrite_with_ready_shooter(&encoded)
        .expect("complete shoot-projectile Deathrite withheld setup");
    ...
    assert!(
        session
            ...
            .expect("paused legal actions")
            .iter()
            .all(|action| action.descriptor["kind"] != "shoot-projectile")
    );
    ...
    let (_, shot) = accept_where(session, |descriptor| {
        descriptor["kind"] == "shoot-projectile" && descriptor["shooterInstanceId"] == shooter_id
    });
