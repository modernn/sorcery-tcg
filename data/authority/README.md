...
The selected local revision is `bundle:official-2026-08-27-v3`, bound to the exact bundle root recorded in `receipts/official-2026-08-27-v3.json`. Runtime consumers select only that immutable ID and receipt root locally; they do not fetch, poll, scrape, serve, or resolve a mutable latest or date alias.

The fixed manual evidence lock is `.local/authority/locks/official-2026-08-27-v3/source-set-lock.json`. Git alone is insufficient to rebuild the revision. Rebuild only from either complete independently verified private source set, preserving the original card API bytes and their source hash; the strict in-memory adapter produces derived project card records and never relabels them as publisher bytes.

The earlier `official-2026-08-20` receipt, private locks, source roots, authorization records, and ignored revision remain unchanged historical evidence and are not selected by current consumers.
