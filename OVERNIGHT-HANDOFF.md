...
`magic_rules.rs` still hosts **88** catalog entries awaiting dedicated backfill (iteration 10 added supplemental high-ID proofs without repointing legacy low-ID entries).
...
None. Phase 3 iteration 10 lanes (`heal-minion`, `grant-airborne`, `destroy-aura`, `return-aura`) merged 2026-09-16.

## Next exact step

1. Continue `magic_rules.rs` backfill for remaining engine-complete families still pointing at the shared file. Suggested iteration 11 four-lane batch: `draw-sites-legacy` (repoint `0200`–`0201` to `draw_sites_rules.rs` proofs `0647`–`0648`), `draw-spells-legacy` (repoint `0196`–`0197` to `draw_spells_rules.rs` proofs `0645`–`0646`), `untap-target-minion-legacy` (repoint four `untapTargetMinion` entries to `untap_magic_rules.rs` proofs `0613`–`0614`), and `tap-target-minion-legacy` (repoint three `tapTargetMinion` entries to `tap_magic_rules.rs` proofs `0611`–`0612`). Alternate lane pair if repoint-only is too thin: `region-magic` (`region`, three Fatality entries) and `burrow-legacy` (four remaining `burrowing` entries). Re-spawn Grok as `cursor/<slug>-0005` from canonical.
