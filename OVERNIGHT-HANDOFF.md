...
`data/rules/catalog.json`: **686 rust-supported / 0 typescript-supported** out of 686.

Latest catalog proofs: Bury burrows loose Artifacts or detaches Avatar-carried Artifacts (`RULE-CATALOG-0679`–`0680`), Rescue offers only own cemetery minions when the opponent also has one (`0681`–`0682`), Mesmerism steals an adjacent minion after the caster moves and excludes two-step targets (`0683`–`0684`), and targeted Magic kills a Deathrite minion as a non-unit source then reaches Death's Door and defeats an Avatar (`0685`–`0686`).

`magic_rules.rs` still hosts **12** catalog entries awaiting dedicated backfill (iteration 15a repointed `0020`, `0021`, `0025`, `0027`, `0029`, `0031`, and `0032` to dedicated high-ID twins; iteration 14 repointed `0019`, `0041`, `0043`, and `0146`; iteration 13 repointed `0057`, `0147`, `0161`, and `0236`; iteration 12 repointed 23 legacy low-ID entries; iteration 11 repointed 16).
...
`cursor/combat-area-repoint-15a-3346`: Phase 3 iteration 15a catalog repoints (`0020`, `0021`, `0025`, `0027`, `0029`, `0031`, `0032` → dedicated `05xx`/`06xx` twins). **12** `magic_rules.rs` rows remain.

## Next exact step

1. Continue `magic_rules.rs` backfill — **12** rows remain. Iteration 15b (Grok lanes): dedicated proofs for rows with no twin yet — `0022`/`0023`/`0024`/`0026`/`0030`/`0033` (combat/area lane leftovers), plus `0014`/`0018` (token-spellcaster harness), and `0042`/`0044`–`0046` (burrow/drown/heal legacy). Re-spawn as `cursor/<slug>-0005` from canonical after 15a merge.
