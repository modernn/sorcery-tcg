...
`data/rules/catalog.json`: **654 rust-supported / 0 typescript-supported** out of 654.

Latest catalog proofs: draw-sites Magic draws hidden Atlas cards and decks out on an empty library (`RULE-CATALOG-0647`–`0648`), target-player life-loss Magic reduces Avatar life and reaches Death's Door without damage (`RULE-CATALOG-0649`–`0650`), target-player life-gain Magic heals an enemy Avatar and cannot leave Death's Door (`RULE-CATALOG-0651`–`0652`), and chosen-discard additional cost pays another hand card then resolves the companion effect (`RULE-CATALOG-0653`–`0654`).

`magic_rules.rs` still hosts **80** catalog entries awaiting dedicated backfill (low-ID entries were not repointed).
...
1. Continue `magic_rules.rs` backfill for remaining engine-complete families still pointing at the shared file. Suggested iteration 9 four-lane batch: `bury-magic` (`burrowing`), `protected-site` (`cannotBeMovedDestroyedOrModified`), `deathrite-magic` (`deathrite`), and `disable-region` (`disabled`). Re-spawn Grok as `cursor/<slug>-0005` from canonical.
