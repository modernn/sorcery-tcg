...
`data/rules/catalog.json`: **275 rust-supported / 0 typescript-supported** out of 275.

Latest catalog proofs: grant-Airborne this turn (`RULE-CATALOG-0274`–`0275`) and cemetery Aura return (`RULE-CATALOG-0272`–`0273`).
...
- `tests/engine` + catalog proofs — 249 engine + 1 catalog pass, including catalog 0167–0275.
...
1. Continue on this branch only. Next high-value official-rules work is grant-Ranged this turn (same temporary-source pattern as Charge and Airborne), leftover 2×2 fail-closed combinations (do not lift Voidwalk or tokens blindly), or more official start/end-turn slices. Atlantean Fate is a different flood that strips other abilities — do not conflate it with Flood. Do not invent MTG keywords.
