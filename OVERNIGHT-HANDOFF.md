...
`data/rules/catalog.json`: **724 rust-supported / 0 typescript-supported** out of 724.

Latest catalog proofs: Fatality Ward plus healthy/Stealth/underground/allied filter matrix (`0721` in `fatality_rules.rs`); temporary +2 power observed Avatar and disabled-minion stats (`0722` in `overpower_rules.rs`); Sparkmage rubble-only nearby activation (`0723` in `sparkmage_rules.rs`); discard-here damage disabled/removed source and oversized footprint counting (`0724` in `discard_random_damage_rules.rs`). Prior iteration: Minor Explosion connected-location topology (`0716` in `minor_explosion_rules.rs`); oversized Bury rubble-as-land and all-Water no-op (`0717`–`0718` in `bury_oversized_rules.rs`); Geomancer Atlas Genesis admission (`0719` in `genesis_draw_rules.rs`); Granary Rats suppression (`0720` in `readiness_affinity_rules.rs`).

`game.rs` catalog refs: **2** (`0721` Fatality filter matrix and `0723` Sparkmage rubble target in `catalog_proofs`). Legacy `game.rs` `#[test]` count: **10** after iteration 22 moved four harness proofs to catalog (Overpower observed stats, Sparkmage rubble target, discard-here source filters; Fatality filter matrix stays in `catalog_proofs`).
...
None. Phase 3 iteration 22 complete on canonical 2026-09-16: `0721` from `cursor/fatality-ward-0721-0005`, `0722` from `cursor/temporary-power-0722-0005`, `0723` from `cursor/sparkmage-rubble-0723-0005`, and `0724` from `cursor/discard-damage-0724-0005`.
