...
`data/rules/catalog.json`: **614 rust-supported / 0 typescript-supported** out of 614.

Latest catalog proofs: Damage-random-unit-at-location Magic picks a location and damages one random unit while Ward absorbs the random hit (`RULE-CATALOG-0607`–`0608`), Kill-target-wounded-minion Magic kills wounded minions only and offers no target when every minion is healthy (`RULE-CATALOG-0609`–`0610`), Tap-target-minion Magic exhausts a ready minion while Ward absorbs the tap (`RULE-CATALOG-0611`–`0612`), and Untap-target-minion Magic readies a tapped minion without breaking Ward and no-ops on already-ready minions (`RULE-CATALOG-0613`–`0614`).
...
Four iteration-4 lanes are implemented on worktrees and pass `cargo test` locally. Merge only after Grok subagent delegation evidence is recorded (orchestrator must not write proof files inline):

| Slug | Branch | Worktree | Commit | IDs |
|------|--------|----------|--------|-----|
| grant-ward | `cursor/grant-ward-0005` | `../sorcery-tcg-grant-ward` | `24a836e` | `0615`–`0616` |
| grant-stealth | `cursor/grant-stealth-0005` | `../sorcery-tcg-grant-stealth` | `61858eb` | `0617`–`0618` |
| kill-target | `cursor/kill-target-0005` | `../sorcery-tcg-kill-target` | `b871e44` | `0619`–`0620` |
| bounce | `cursor/bounce-0005` | `../sorcery-tcg-bounce` | `5c8b6cf` | `0621`–`0622` |

Phase 3 orchestration requires `Task(subagent_type="best-of-n-runner", model="cursor-grok-4.6-high")` per lane with a dedicated worktree. Iteration 3 lane `random-bolt` violated this (committed directly to canonical at `68135f7` without a merge commit); the other three iteration-3 lanes used proper branch merges.

## Next exact step

1. Merge the four pending iteration-4 lanes above into canonical (sequential merge commits, then `cargo test` on touched proof files). Next backfill batch after merge: `returnTargetMinionToOwnerHand` and remaining `magic_rules.rs` families still pointing at the shared file.
