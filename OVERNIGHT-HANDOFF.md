...
`data/rules/catalog.json`: **638 rust-supported / 0 typescript-supported** out of 638.

Latest catalog proofs: Return-artifact Magic returns carried Artifacts to hidden hand (`RULE-CATALOG-0631`–`0632`), Destroy-artifact Magic moves Artifacts to cemetery (`RULE-CATALOG-0633`–`0634`), Cemetery Magic return restores own cemetery Magic to hidden hand (`RULE-CATALOG-0635`–`0636`), and Cemetery Artifact return restores own cemetery Artifact to hidden hand (`RULE-CATALOG-0637`–`0638`).
...
1. Continue `magic_rules.rs` backfill for remaining engine-complete families still pointing at the shared file. Suggested iteration 7 four-lane batch: `cemetery-site-return` (`returnTargetSiteFromOwnCemetery`), `cemetery-aura-return` (`returnTargetAuraFromOwnCemetery`), `target-player-discard` (`targetPlayerDiscardsCards`), and `draw-spells` (`drawSpells`). Re-spawn Grok as `cursor/<slug>-0005` from canonical.
