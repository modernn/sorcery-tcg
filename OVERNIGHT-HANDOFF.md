...
Canonical line: `master` @ `de62ffa`. Lanes through `RULE-CATALOG-0480` are merged (2×2 token/wrap/outer-column/deck-pair, oversized start-turn random teleport, full token Genesis, Chain Magic footprint hops, and stacked start-turn draw+mill). Branch new Phase 3 work as `cursor/<one-family>-0005` from here. Do not grow already-merged landings.
