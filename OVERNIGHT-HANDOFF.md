...
`data/rules/catalog.json`: **714 rust-supported / 0 typescript-supported** out of 714.

Latest catalog proofs: Water replacement underwater Deathrite source region and Genesis resume after ordered Deathrites (`RULE-CATALOG-0713`–`0714` in `water_settlement_rules.rs`). Prior iteration: Chain Magic low-mana and same-region hop exclusion (`0709` in `chain_magic_rules.rs`); Bury ordered static Deathrite deferral with checkpoint round-trip (`0710` in `burrow_settlement_rules.rs`); Duel Avatar ally strike routing (`0711` in `duel_rules.rs`); underground first-strike Duel deathrite-order checkpoint through terminal (`0712` in `duel_rules.rs`).

`game.rs` catalog refs: **0** (iteration 17 extracted the last four harness proofs). Legacy `game.rs` `#[test]` wrappers remain until follow-up pruning; iteration 20 lane A pruned six redundant wrappers and lane B moved Water settlement proofs to catalog.
...
Phase 3 iteration 20 partial landed on canonical 2026-09-16: `0713`–`0714` from `cursor/water-settlement-0713-0005` and six `game.rs` wrapper prunes from `cursor/game-rs-prune-0005`. **`RULE-CATALOG-0715` oversized attack contested cell** (`cursor/oversized-attack-0715-0005`) remains pending.

## Next exact step

1. Merge `RULE-CATALOG-0715` oversized attack contested cell from `cursor/oversized-attack-0715-0005`, then continue broader rules coverage per `.planning/phases/03-complete-core-game-rules/03-PROGRESS.md` (terrain, Storyline, combat tiers, authority verification). **Phase 3 is not complete** until those gates are green.
