...
`data/rules/catalog.json`: **708 rust-supported / 0 typescript-supported** out of 708.

Latest catalog proofs: zero-domain site recovery (`RULE-CATALOG-0701`–`0702`), Sinkhole subsurface preservation (`0703` land/protected/rubble; `0704` Water subsurface), Craterize discard/destroy/damage grid (`0705` happy path; `0706` discard-cost refusal plus protected-site damage), and Raise Dead cemetery selection plus free placement (`0707` random pick/free place; `0708` blocked 2×2 footprint). Low IDs `0007`, `0012`, `0159`, and `0160` repoint to those high-ID twins.

`game.rs` catalog refs: **0** (iteration 17 extracted the last four harness proofs). Legacy `game.rs` `#[test]` wrappers remain until follow-up pruning.

`magic_rules.rs` catalog refs: **0**. Orphan `#[test]` count: **4** (pruned iteration 18; 833 lines). Kept without catalog coverage:

- `chain_magic_should_require_mana_and_same_region_hops` — low-mana suppression and underground hop exclusion
- `bury_should_defer_completion_until_ordered_static_deathrites_finish` — ordered Deathrite deferral and checkpoint resume
- `duel_should_apply_an_avatar_ally_from_the_existing_checkpoint` — Avatar ally from shared Duel checkpoint
- `duel_should_checkpoint_an_underground_first_strike_and_finish_before_terminal` — underground first strike, ordered Deathrites, terminal completion
...
None. Phase 3 iteration 17 (`0701`–`0708` on `game.rs`), iteration 18 `magic_rules.rs` prune, and alias-twins split (`cursor/alias-twins-0005` → canonical) landed 2026-09-16. Catalog `game.rs` and `magic_rules.rs` refs are both **0**.

## Next exact step

1. Backfill catalog coverage for the four kept `magic_rules.rs` harness proofs above or accept them as permanent non-catalog guards. Delete redundant `game.rs` harness wrappers once stable. Phase 3 catalog backfill is done; remaining Phase 3 work is broader rules coverage per `.planning/phases/03-complete-core-game-rules/03-PROGRESS.md` (terrain, Storyline, combat tiers, authority verification). Do not mark Phase 3 complete until those gates are green.
