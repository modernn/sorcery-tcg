...
1. Continue `magic_rules.rs` backfill for remaining engine-complete families (`grantWardToTargetMinion`, `grantStealthToTargetMinion`, `killTargetMinion`, `returnTargetMinionToOwnerHand`, destroy/return site and artifact families, mill/discard families still pointing at `magic_rules.rs`). Suggested iteration 4 four-lane batch: `grant-ward` (`0615`–`0616`), `grant-stealth` (`0617`–`0618`), `kill-target` (`0619`–`0620`), `bounce` (`0621`–`0622`).
