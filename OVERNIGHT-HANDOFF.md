...
`data/rules/catalog.json`: **720 rust-supported / 0 typescript-supported** out of 720.

Latest catalog proofs: Minor Explosion connected-location topology with oversized footprints and underground isolation (`0716` in `minor_explosion_rules.rs`); oversized Bury rubble-as-land eligibility and all-Water paid no-op (`0717`–`0718` in `bury_oversized_rules.rs`); Geomancer Atlas Genesis self-play admission (`0719` in `genesis_draw_rules.rs`); Granary Rats void/underground/partial-disable suppression renumbered to `0720` (`readiness_affinity_rules.rs`). Prior iteration: oversized attack contested cell (`0715` in `footprint_rules.rs`); Water replacement Deathrite region and Genesis resume (`0713`–`0714` in `water_settlement_rules.rs`).

`game.rs` catalog refs: **0** (iteration 17 extracted the last four harness proofs). Legacy `game.rs` `#[test]` count: **14** after iteration 21 moved four harness proofs to catalog (minor explosion, bury oversized, geomancer admission, granary suppression).
...
None. Phase 3 iteration 21 complete on canonical 2026-09-16: `0716` from `cursor/minor-explosion-0716-0005`, `0717`–`0718` from `cursor/bury-oversized-0717-0005`, `0719` from `cursor/geomancer-admission-0719-0005`, and `0720` (renumbered from granary branch `0718`) from `cursor/granary-rats-0718-0005`.
