...
`data/rules/catalog.json`: **715 rust-supported / 0 typescript-supported** out of 715.

Latest catalog proofs: Oversized 2×2 overlapping-footprint declare-attack chooses the lowest shared contested cell (`RULE-CATALOG-0715` in `footprint_rules.rs`); Water replacement underwater Deathrite source region and Genesis resume after ordered Deathrites (`0713`–`0714` in `water_settlement_rules.rs`). Prior iteration: Chain Magic low-mana and same-region hop exclusion (`0709` in `chain_magic_rules.rs`); Bury ordered static Deathrite deferral with checkpoint round-trip (`0710` in `burrow_settlement_rules.rs`); Duel Avatar ally strike routing (`0711` in `duel_rules.rs`); underground first-strike Duel deathrite-order checkpoint through terminal (`0712` in `duel_rules.rs`).

`game.rs` catalog refs: **0** (iteration 17 extracted the last four harness proofs). Legacy `game.rs` `#[test]` count: **18** after iteration 20 lane A pruned six redundant wrappers, lane B moved Water settlement proofs to catalog, and lane C moved the oversized attack contested-cell proof to catalog.
...
None. Phase 3 iteration 20 complete on canonical 2026-09-16: `0713`–`0714` from `cursor/water-settlement-0713-0005`, six `game.rs` wrapper prunes from `cursor/game-rs-prune-0005`, and `0715` oversized attack contested cell from `cursor/oversized-attack-0715-0005`.
