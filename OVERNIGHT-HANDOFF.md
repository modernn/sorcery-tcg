...
`magic_rules.rs` still hosts **27** catalog entries awaiting dedicated backfill (iteration 12 repointed 23 legacy low-ID entries to existing high-ID dedicated proofs; iteration 11 repointed 16; iteration 10 added supplemental proofs without repointing). Skipped `0236` — no high-ID twin for atlas-discard additional cost yet.
...
None. Phase 3 iteration 12 (mill/cemetery-return/kill-bounce/grant-ward-stealth/discard/site-artifact legacy repoints) landed 2026-09-16 on canonical.

## Next exact step

1. Continue `magic_rules.rs` backfill. Suggested iteration 13 new-family batch: `chosen-discard-atlas` (add `0655`-class proof in `chosen_discard_cost_rules.rs`, repoint `0236`), `region-magic`/`0147` (`fatality_rules.rs`), `burrow-legacy` (`0042`–`0044`/`0043` → `bury_magic_rules.rs`/`cave_in_rules.rs`), card-specific combat/area (`0020`–`0031`, `0023`–`0027`, `0029`–`0033`), `rescue`/`0041`, `genesis-strike`/`0057`, `mesmerism`/`0146`, `0161`, and early harness rows (`0014`, `0018`, `0019`). Re-spawn as `cursor/<slug>-0005` from canonical.
