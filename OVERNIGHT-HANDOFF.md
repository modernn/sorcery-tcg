...
Live line: `cursor/ballista-helper-occupy-0005`. Stack this on current `master` (`b5c626d`). Do not grow already-merged landings.

This branch fixes tap-pair Artifact helpers so "standing with" the bearer is footprint overlap, not only the remembered carried cell. A B3-anchored 2×2 carrying a Siege Ballista at C4 can tap a ready ally on B3; an ally on B2 cannot. Measured Ballista range still walks from the remembered cell. Flood/Drought water-cast proofs, Updraft Ridge occupy, measured-range Magic, and Secret Tunnel hops stay unchanged. Compact demo/batch stdout is unchanged.
