...
Canonical line: `master` @ `178953d`. Lanes through `RULE-CATALOG-0382` are merged (2×2 token/wrap/outer-column/deck-pair, oversized start-turn random teleport, and non-choice token Genesis). Branch new Phase 3 work as `cursor/<one-family>-0005` from here. Do not grow already-merged landings.
...
2×2 Voidwalk is bound (`RULE-CATALOG-0316`–`0317`). Token occupancy (`0371`–`0372`), top/bottom wraparound (`0373`–`0374`), outer-column casts (`0375`–`0376`), deck-pair harness (`0377`–`0378`), oversized start-turn random teleport (`0379`–`0380`), and non-choice token Genesis draw-site/disable (`0381`–`0382`) are admitted. Token adjacent-damage choice Genesis remains fail-closed separately.
