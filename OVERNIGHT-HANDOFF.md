...
Live line: `cursor/ballista-range-occupy-0005`. Stack this on current `master` (`21d0e64`). Do not grow already-merged landings.

This branch measures tap-pair Artifact range from every cell the bearer occupies, not only the remembered carried cell. A B3-anchored 2×2 carrying a Siege Ballista at B3 reaches D4 (one step from occupied C4) and cannot reach E3 (three steps from every occupied cell). Helper “standing with” remain footprint overlap. Flood/Drought water-cast proofs, Updraft Ridge occupy, measured-range Magic, and Secret Tunnel hops stay unchanged. Compact demo/batch stdout is unchanged.
