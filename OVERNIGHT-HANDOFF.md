...
`data/rules/catalog.json`: **694 rust-supported / 0 typescript-supported** out of 694.

Latest catalog proofs: token Magic summons in cell order with deterministic identities and dead tokens banish (`RULE-CATALOG-0687`–`0688`), printed Spellcasters cast while summoning sick or tapped (`0689`–`0690`), Bury/Cave-In burrow settlement and Deathrite ordering (`0691`–`0692`), and Drown kills non-Submerge minions at Water sites while healController caps and cannot leave Death's Door (`0693`–`0694`).

`magic_rules.rs` still hosts **6** catalog entries awaiting dedicated backfill: `0022`, `0023`, `0024`, `0026`, `0030`, and `0033`. Iteration 15b repointed `0014`, `0018`, `0042`, `0044`, `0045`, and `0046` to dedicated high-ID twins; iteration 15a repointed `0020`, `0021`, `0025`, `0027`, `0029`, `0031`, and `0032`.
...
None. Phase 3 iteration 15b (four Grok lanes: token summon, spellcaster harness, burrow settlement, drown/heal legacy; `0687`–`0694`) landed 2026-09-16 on canonical. **6** `magic_rules.rs` rows remain.

## Next exact step

1. Continue `magic_rules.rs` backfill — **6** rows remain (`0022`, `0023`, `0024`, `0026`, `0030`, `0033`). Iteration 16 (Grok lanes, three two-entry families from canonical): `cursor/leap-chain-0005` (`0022` Leap Attack Deathrite resume + `0030` Chain Magic hops), `cursor/region-target-0005` (`0023` caster-region Stealth filter + `0026` subsurface disable settlement), and `cursor/lash-overpower-0005` (`0024` Lash damage/untap + `0033` Overpower until End Phase). Each lane adds `0695`–`0700` twins and repoints the low IDs. Phase 3 is not complete until `magic_rules.rs` is empty and every catalog row has a dedicated twin.
