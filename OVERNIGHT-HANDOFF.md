...
`data/rules/catalog.json`: **630 rust-supported / 0 typescript-supported** out of 630.

Latest catalog proofs: Destroy-site Magic replaces sites with Rubble and respects protected sites (`RULE-CATALOG-0623`–`0624`), Return-site Magic returns sites to Atlas and respects protected sites (`RULE-CATALOG-0625`–`0626`), Mill-spells Magic mills Spellbook tops or no-ops on empty libraries (`RULE-CATALOG-0627`–`0628`), and Mill-sites Magic mills Atlas tops or no-ops on empty Atlases (`RULE-CATALOG-0629`–`0630`).
...
1. Continue `magic_rules.rs` backfill for remaining engine-complete families still pointing at the shared file. Suggested iteration 6 four-lane batch: `return-artifact` (`returnTargetArtifactToOwnerHand`), `destroy-artifact` (`destroyTargetArtifact`), plus two cemetery-return families from the next tier. Re-spawn Grok as `cursor/<slug>-0005` from canonical.
