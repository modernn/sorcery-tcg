...
Canonical line: `cursor/source-bound-control-0005` @ `20c09f9`. Lanes through `RULE-CATALOG-0520` are on this branch. TypeScript admits 2×2 token/wrap/outer-column (`0371`–`0376`); 2×2 Landbound composes with Flood and Drought (`0507`–`0508`, `0511`); 2×2 Waterbound composes with Drought and Flood (`0509`–`0510`, `0512`); this-turn, stealth-bound, source-bound Genesis, and sacrifice-to-steal Artifact control are bound (`0513`–`0520`). Branch new Phase 3 work as `cursor/<one-family>-0005` from here. Do not grow already-merged landings.
