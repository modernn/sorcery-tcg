...
`data/rules/catalog.json`: **662 rust-supported / 0 typescript-supported** out of 662.

Latest catalog proofs: burrow-target-minion-or-artifact Magic burrows then region-kills on Earth and no-ops on Water (`RULE-CATALOG-0655`–`0656`), Craterize-style Magic destroys or is prevented on protected sites while still dealing grid damage (`0657`–`0658`), Mesmerism transfers control and Deathrite then respects nearby targeting (`0659`–`0660`), and Freeze disable-until-next-turn Magic disables a nearby minion or is absorbed by Ward (`0661`–`0662`).

`magic_rules.rs` still hosts **66** catalog entries awaiting dedicated backfill (iteration 9 added supplemental high-ID proofs without repointing legacy low-ID entries).
...
1. Continue `magic_rules.rs` backfill for remaining engine-complete families still pointing at the shared file. Suggested iteration 10 four-lane batch: `draw-sites-magic` (`drawSites`, four legacy entries), `draw-spells-magic` (`drawSpells`), `region-magic` (`region`, three entries), and `burrow-legacy` (repoint remaining `burrowing` entries from shared `magic_rules.rs`). Re-spawn Grok as `cursor/<slug>-0005` from canonical.
