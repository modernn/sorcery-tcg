...
`data/rules/catalog.json`: **670 rust-supported / 0 typescript-supported** out of 670.

Latest catalog proofs: heal-target-minion Magic clears same-turn damage or no-ops on undamaged minions and excludes Avatars (`RULE-CATALOG-0663`–`0664`), grant-Airborne supplemental proofs cover grounded and already-Airborne minions through End Phase (`0665`–`0666`), destroy-target-aura Magic sends Flood to cemetery or offers no target (`0667`–`0668`), and return-target-aura Magic bounces Flood to hand or offers no target (`0669`–`0670`).

`magic_rules.rs` still hosts **50** catalog entries awaiting dedicated backfill (iteration 11 repointed 16 legacy low-ID entries to existing high-ID dedicated proofs; iteration 10 added supplemental proofs without repointing).
...
None. Phase 3 iteration 11 (`draw-spells/sites`, `tap/untap`, protected-site, return-artifact, target-player life, mill-spells legacy repoints) merged 2026-09-16.

## Next exact step

1. Continue `magic_rules.rs` backfill. Suggested iteration 12 four-lane batch: `mill-legacy` (repoint `0226`–`0228` to `mill_spells_rules.rs`/`mill_sites_rules.rs` proofs `0627`–`0630`), `cemetery-return-legacy` (repoint `0229`–`0234` to `cemetery_*_return_rules.rs` proofs `0635`–`0640`), `kill-bounce-legacy` (repoint `0198`–`0199`/`0202`–`0203` to `kill_minion_rules.rs`/`return_minion_rules.rs` proofs `0619`–`0622`), and `grant-ward-stealth-legacy` (repoint `0221`–`0224` to `grant_ward_rules.rs`/`grant_stealth_rules.rs` proofs `0615`–`0618`). Alternate if repoint-only thins: `discard-legacy` (`0235`/`0236`/`0239`/`0240` → `0653`–`0654`/`0643`–`0644`) plus `site-artifact-legacy` (`0207`/`0209`/`0211` → `0623`/`0625`/`0633`). New proof families only where no high-ID dedicated file exists yet (`region-magic`/`0147`, `burrow-legacy`/`0042`–`0044`/`0043`, card-specific `0020`–`0031`). Re-spawn as `cursor/<slug>-0005` from canonical.
