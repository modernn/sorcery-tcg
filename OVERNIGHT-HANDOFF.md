...
`data/rules/catalog.json`: **728 rust-supported / 0 typescript-supported** out of 728.

Latest catalog proofs: Tower controller-agnostic derived stats (`0725` in `tower_rules.rs`); oversized carried Artifact end-turn life loss cell attribution (`0726` in `artifact_life_loss_rules.rs`); Pick Up/Drop locality and Disable filters (`0727` in `artifact_carry_rules.rs`); end-turn air-threshold cast counter reset for both players (`0728` in `phase_storyline_rules.rs`). Prior iteration: Fatality Ward plus healthy/Stealth/underground/allied filter matrix (`0721` in `fatality_rules.rs`); temporary +2 power observed Avatar and disabled-minion stats (`0722` in `overpower_rules.rs`); Sparkmage rubble-only nearby activation (`0723` in `sparkmage_rules.rs`); discard-here damage disabled/removed source and oversized footprint counting (`0724` in `discard_random_damage_rules.rs`).

`game.rs` catalog refs: **5** (`0720` Granary Rats, `0721` Fatality filter matrix, `0723` Sparkmage rubble target, `0727` Pick Up/Drop, `0728` air-threshold reset in `catalog_proofs`). Legacy `game.rs` `#[test]` count: **6** after iteration 23 moved four harness proofs to catalog (Tower stats, oversized Artifact life loss, Pick Up/Drop, air-threshold reset).
...
None. Phase 3 iteration 23 complete on canonical 2026-09-16: `0725` from `cursor/tower-controller-0725-0005`, `0726` from `cursor/artifact-life-0726-0005`, `0727` from `cursor/pickup-drop-0727-0005`, and `0728` from `cursor/air-threshold-0728-0005`.
