...
`data/rules/catalog.json`: **646 rust-supported / 0 typescript-supported** out of 646.

Latest catalog proofs: Cemetery Site return restores own cemetery Site to hidden Atlas (`RULE-CATALOG-0639`–`0640`), Cemetery Aura return restores own cemetery Aura to hidden hand (`RULE-CATALOG-0641`–`0642`), target-player discard lets the targeted player choose then no-ops with an empty hand (`RULE-CATALOG-0643`–`0644`), and draw-spells Magic draws hidden Spellbook cards then decks out on an empty library (`RULE-CATALOG-0645`–`0646`).
...
1. Continue `magic_rules.rs` backfill for remaining engine-complete families still pointing at the shared file. Suggested iteration 8 four-lane batch: `draw-sites` (`drawSites`), `target-player-life-loss` (`targetPlayerLosesLife`), `target-player-life-gain` (`targetPlayerGainsLife`), and `chosen-discard-cost` (`discardCardAsAdditionalCost`). Re-spawn Grok as `cursor/<slug>-0005` from canonical.
