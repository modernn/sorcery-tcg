...
Canonical line: `master` @ `3617c64`. Lanes through `RULE-CATALOG-0498` are merged (2×2 token/wrap/outer-column/deck-pair, oversized start-turn random teleport, full token Genesis, Chain Magic footprint hops, stacked start-turn draw+mill, and mixed unconditional plus first-copy-only site Genesis mana). Branch new Phase 3 work as `cursor/<one-family>-0005` from here. Do not grow already-merged landings.
