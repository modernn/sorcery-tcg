...
`data/rules/catalog.json`: **606 rust-supported / 0 typescript-supported** out of 606.

Latest catalog proofs: Leap-attack-ally Magic steps an ally and strikes at the destination and restricts immobile allies to stay-and-strike (`RULE-CATALOG-0599`–`0600`), Lure-enemy-minion Magic tempts a nearby enemy closer and no-ops without a target (`RULE-CATALOG-0601`–`0602`), Fight-ally-with-adjacent-enemy Magic fights through the shared pipeline and breaks Ward without combat (`RULE-CATALOG-0603`–`0604`), and Damage-each-aboveground-minion Magic kills surface minions while Ward absorbs the hit (`RULE-CATALOG-0605`–`0606`).

`magic_rules.rs` still hosts **76** catalog entries awaiting dedicated backfill (low-ID entries were not repointed).
...
1. Continue `magic_rules.rs` backfill for remaining engine-complete families (`damageRandomUnitAtLocation`, `killTargetWoundedMinion`, tap/untap/grant/bounce/destroy/return/mill/discard families still pointing at `magic_rules.rs`).
