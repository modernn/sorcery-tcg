...
Canonical line: `master` @ `288cdaa`. Lanes through `RULE-CATALOG-0380` are merged (2×2 token/wrap/outer-column/deck-pair plus oversized start-turn random teleport). Branch new Phase 3 work as `cursor/<one-family>-0005` from here. Do not grow already-merged landings.
...
2×2 Voidwalk is bound (`RULE-CATALOG-0316`–`0317`). Token occupancy (`0371`–`0372`), top/bottom wraparound (`0373`–`0374`), outer-column casts (`0375`–`0376`), deck-pair harness (`0377`–`0378`), and oversized start-turn random teleport (`0379`–`0380`) are admitted. Token Genesis on token cards remains fail-closed separately.
...
`data/rules/catalog.json`: **380 rust-supported / 0 typescript-supported** out of 380.
