...
Latest catalog proofs: zero-domain site recovery (`RULE-CATALOG-0701`–`0702`), Sinkhole subsurface preservation (`0703`–`0704`), Craterize discard/destroy/damage grid (`0705`–`0706`), and Raise Dead cemetery selection plus free placement (`0707`–`0708`). Low IDs `0007`, `0012`, `0159`, and `0160` now repoint to those high-ID twins.
...
`magic_rules.rs` catalog refs: **0**. Orphan `#[test]` count: **69** (not pruned this iteration).
...
None. Phase 3 iteration 17 (last four `game.rs` catalog proofs; `0701`–`0708`) landed 2026-09-16 on canonical. Catalog `game.rs` and `magic_rules.rs` refs are both **0**.

## Next exact step

1. Iteration 18 — split alias catalog twins `0704`, `0706`, and `0708` into genuinely distinct proof slices (Sinkhole water subsurface, Craterize protected-site/cost-only, Raise Dead placement/blocked-footprint). Prune `magic_rules.rs` orphan tests already duplicated in dedicated `*_rules.rs` files; delete redundant `game.rs` harness wrappers once stable. Phase 3 catalog backfill is done; remaining Phase 3 work is broader rules coverage per `.planning/phases/03-complete-core-game-rules/03-PROGRESS.md` (terrain, Storyline, combat tiers, authority verification). Do not mark Phase 3 complete until those gates are green.
